...
#include "halFormatter.h"   //HICC2_Doulas_0117
#include "halMCUCtrlAPI.h"
#include "halGui.h"

#define BIST_KeyCheck_OFF 0   //HICC2_Steven_0022 start
...
static UINT8 m_ucBIST_KeyStatus = BIST_KeyCheck_OFF; //HICC2_Steven_0022 end
...
static UINT8 m_ucMotorLensAPPEnable = 0; //HICC2_Casper_0030
static sLENSMEMORY        m_sLensMem = {0}; //R70PD_AC_0105 //R70KPD_VSC_Wesley_0003 //HICC2_Casper_0030

static UINT8 m_ucLensType = 0; //HICC2_Julie_0071//H30K_Julie_0005
static UINT8 m_ucFujiLensDetect = 0; //HICC2_Julie_0071//H30K_Julie_0005
...
static void appInput_Motor_Process(eKEY_LIST ekey) //A70LV_Larry_0219
{
    switch(ekey)
    {
        ...
        case keHOLD_ZOOMOUT:
            ...
            //utilCounterSet(eCOUNTER_TYPE_APP_LENS, 50); //HICC2_Casper_0030 //HICC2_Casper_0032 //HICC2_Casper_0041
            break;
        ...
        case keHOLD_LENSHLEFT:
            ...
            m_ucMotorLensAPPEnable = m_ucMotorLensAPPEnable | 0x01; //bit 0 = H
            //utilCounterSet(eCOUNTER_TYPE_APP_LENS, 50); //HICC2_Casper_0030 //HICC2_Casper_0032 //HICC2_Casper_0041
            break;
        ...
        case keHOLD_LENSVDOWN:
			...
			m_bMotorZoomEnale = FALSE;
            m_bMotorLensEnale = TRUE;
            m_ucMotorLensAPPEnable = m_ucMotorLensAPPEnable | 0x02; //bit 1 = V
            //utilCounterSet(eCOUNTER_TYPE_APP_LENS, 50); //HICC2_Casper_0030 //HICC2_Casper_0032 //HICC2_Casper_0041
            break;
...
void appKeypadBacklighthandle(sKEY_DATA *psInputKey)
{
#if (0)
    UINT32 dwLED = 0;
    ...
    if(psInputKey->eKeyType == eKEYINPUT_KEYPAD)
    {
        switch(psInputKey->wKeyCode)
        {
         ...
         appLedProc_SingleLed_Set(dwLED);
    }
#else
...
// ==============================================================================
// FUNCTION NAME: appInput_Uart_Switch_Process
// DESCRIPTION:
//
//
// Params:
...
// --------------------
// ==============================================================================
static void appInput_Uart_Switch_Process(eKEY_LIST ekey)
{
	UINT8 ucIndex = 0;
    switch(ekey)
    {
        ...
        case keHOLD_UART_TO_NXP:
            ...
            if(0 == utilCounterGet(eCOUNTER_TYPE_HOTKEY))
            {
                ...
                palDataMgr_Data_Access(edcUART_SWITCH, edaWRITE_THROUGH_WITH_ACTION, &ucIndex); //appGui_DataCode_Value_Set(edcUART_SWITCH, edaWRITE_THROUGH_WITH_ACTION, 0);
...
eEXEC_CODE palInputProc_Init(void)
{
    ...
    palSysCtrlMgr_Baudrate_Set(ucValue); //HICC2_Steven_0026
    //halBoard_Baudrate_Set(ucValue);                                             //G100_Owen_0012
...
eEXEC_CODE palInputProc_PowerNormal(void)       //G100_Owen_0007
{
    ...
    palDataMgr_Data_Access(edcREAR_IR, edaREAD, (void*)&ucValue);
    palDataMgr_Data_Access(edcREAR_IR, edaWRITE_RAM_ONLY_WITH_ACTION, (void*)&ucValue);
    ...
    m_cDisableKey = 0;
    m_cUSTTestPattern = 0;
    m_cFOTA_Status = eFOTA_SYSTEM_UPDATE_STATUS_IDLE;

    return eEXEC_CODE_PASS;
}
// ==============================================================================
// FUNCTION NAME: palInputProc_BUFFERINSERT
// DESCRIPTION:
//
//
// Params:
...
{
    sKEY_DATA sKeyData = *(sKEY_DATA *)pvKey;

    palInputProc_OPDKeyEvent(sKeyData);  //A35G2_Simon_0075

    if(sKeyData.eKeyEvent == eKEY_EVENT_RELEASE)  //H2PF_Simon_0156
    {
        utilQueueFlush(&m_sInputRingBuffer);
    }
...
// ==============================================================================
// FUNCTION NAME: palInputProc_TwistKeyEvent
// DESCRIPTION:
//
//
// Params:
...
// Modification History
// --------------------
// 2023/04/20, Larry Create
// --------------------
// ==============================================================================
void palInputProc_TwistKeyEvent(sKEY_DATA *psInputKey)
{
    switch(psInputKey->wKeyCode)
    {
        case keZOOMIN:
        ...
        case keHOLD_LENSHLEFT:
            LOG_MSG(db_APP_INPUTKEY, "\r\n keHOLD_LENSHLEFT");
            if( psInputKey->eKeyType == eKEYINPUT_LAN )
            {
                #ifdef LAN_MOTOR_KEY_DOUBLE_RUN   //HICC2_Simon_0004
                palMotorLensSet(eLENS_CMDS_RUN_LEFT_DOUBLE); //HICC2_Doulas_0105
                #else
            ...
            {
                palMotorLensSet(eLENS_CMDS_RUN_LEFT); //HICC2_Doulas_0105
            }
            break;
        ...
        case keHOLD_LENSHRIGHT:
            ...
            {
                #ifdef LAN_MOTOR_KEY_DOUBLE_RUN   //HICC2_Simon_0004
                palMotorLensSet(eLENS_CMDS_RUN_RIGHT_DOUBLE); //HICC2_Doulas_0105
                #else
            ...
            {
                palMotorLensSet(eLENS_CMDS_RUN_RIGHT); //HICC2_Doulas_0105
            }
            break;
        ...
        case keHOLD_LENSVUP:
            ...
            {
                #ifdef LAN_MOTOR_KEY_DOUBLE_RUN   //HICC2_Simon_0004
                palMotorLensSet(eLENS_CMDS_RUN_UP_DOUBLE); //HICC2_Doulas_0105
                #else
            ...
            {
                palMotorLensSet(eLENS_CMDS_RUN_UP); //HICC2_Doulas_0105
            }
            break;
        ...
        case keHOLD_LENSVDOWN:
            ...
            {
                #ifdef LAN_MOTOR_KEY_DOUBLE_RUN   //HICC2_Simon_0004
                palMotorLensSet(eLENS_CMDS_RUN_DOWN_DOUBLE); //HICC2_Doulas_0105
                #else
            ...
            {
                palMotorLensSet(eLENS_CMDS_RUN_DOWN); //HICC2_Doulas_0105
            }
            break;
    }
}
...
static BOOL palInputProc_KeyFilter(sKEY_DATA *psInputKey)
{
	...
	palDataMgr_Data_Access(edcFACTORY_TEST_PATTERN, edaREAD, &ucFactoryTP);
	palDataMgr_Data_Access(edcLENS_CALIBRATION, edaREAD, &ucLensCalibration);
    ...
    if(palInputProc_Keypad_Lock_Input_Check(psInputKey) == TRUE)
    {
        bEnable = FALSE;
        if(palInputProc_Keypad_UnLock_Input_Check(psInputKey) == TRUE)
        {
            palDataMgr_UI_EventSend(edcUI_EVENT_KEYPAD_LOCK, 0, NULL);
        }
        else
        {
            palDataMgr_UI_EventSend(edcUI_EVENT_KEYPAD_LOCK, 1, NULL);
        }
    }
    else
#endif
	...
	else if(palInputProc_Quick_Key_Lock_Input_Check(psInputKey) == FALSE)// HICC2_Bruce_0011
	{
		bEnable = FALSE;
	}
#endif
    ...
    else if((palGeo_ApLinkFlag_Get() == TRUE) && (TRUE == palGeo_IsDraw_OSD(eWDT_BEFORE_WARP)))
    {
        switch(psInputKey->wKeyCode)
        {
            //Power Off
            ...
            case keHOLD_POWERON:
            case keSTANDBY:
            //case keINPUT:  //H2PF_Simon_0065
                bEnable = TRUE;
                palGeo_OSD_On(eWIL_AFTER_WARP);
                palGeo_RestorePalette();
                if(palDataMgr_CurTestPatternGet() != eTID_OFF)
                {
                    UINT8  ucData = eCM_TEST_PATTERN_OFF;
            ...
            default:
                palInputProc_TwistKeyEvent(psInputKey);
                break;
        }
    }
    #endif
    ...
    else if(palDataMgr_ACU_Target_Status_Get() == ets_ON)  //ACU Select Target  //H2PF_Simon_0035
    {
    ...
    else if((palDataMgr_CurTestPatternGet() != eTID_OFF)
    && (palDataMgr_CurTestPatternGet() < eTID_HSG_RED)
    && (ucFactoryTP == eCM_FACTORY_TP_NONE))       //A70LV_Doulas_0230 Test pattern on //A35G2_BRC_Casper_0115 //HICC2_Casper_0012 //HICC2_Casper_0019
    {
        ...
        switch(psInputKey->wKeyCode)
        {
            ...
            case keSTANDBY:
                if(palInputProc_KeyTimerHander(psInputKey) == TRUE)
                {
                    ...
                    ucTP = eTID_OFF; //HICC2_Casper_0012
            ...
            case keLEFT: //HICC2_Casper_0012
                if(palInputProc_KeyTimerHander(psInputKey) == TRUE)
                {
                    ...
                    else
                    {
            			...
            			//appGui_TestPatternFocusIndexEvent(psInputKey->eKeyType, ekLEFT); //G100_Casper_0006 //###
                    }
                }
                bEnable = FALSE;
                break;
    ...
    {
        //BOOL bServiceMode = FALSE;//appDataMgr_ServiceModeGet();    //ZU860_Doulas_0031 modify
        UINT8 ucTP;
        switch(psInputKey->wKeyCode)
        {
            ...
            case keBACK:
                ...
                palDataMgr_Data_Access((eDATA_CODE)edcUST_Pattern, edaWRITE_RAM_ONLY_WITH_ACTION, &ucTP);
                palDataMgr_UI_EventSend(edcUI_EVENT_OSD_EXIT, TRUE, NULL); //appGui_CloseMenu();
                ...
                //appGui_Send_MenuOpen_UST_Message2(); //###
                #endif
    ...
    else if(ucLensCalibration)
    {
        if((psInputKey->wKeyCode == keINPUT) || (psInputKey->wKeyCode == keTEST) || (psInputKey->wKeyCode == keSWAP))
        {
            LOG_MSG(db_APP_INPUTKEY, "Key Block <%d>\n", __LINE__);
            bEnable = FALSE;
        }
        else
        {
    ...
    else if((palInput_FOTA_StatusGet() == eFOTA_SYSTEM_UPDATE_STATUS_DOWNLOADING) ||
            (palInput_FOTA_StatusGet() == eFOTA_SYSTEM_UPDATE_STATUS_UPGRADE_READY))
    {
        if(psInputKey->wKeyCode == keMUTE)
        {
            bEnable = TRUE;
        }
        else
...
//HICC2_Tim_0004, add, ***
#define ucOE_JIG_PWM_RANK               5
#define ucOE_JIG_PWM_LD_NO              4
static UINT8 ucOE_JIG_PWM_Stage = ucOE_JIG_PWM_RANK;
UINT16 uiOE_JIG_PWM_TABLE[ucOE_JIG_PWM_RANK][ucOE_JIG_PWM_LD_NO]=
{
    {190, 190, 190, 190},
    {300, 300, 300, 300},
    {400, 400, 400, 400},
    ...
    switch(sInputKey.wKeyCode)
    {
        ...
        case ke1:
            if(appGui_Get_MenuState_IsOSD_Open())
            {
                ...
                {
                    appGui_SendKeyEvent(ucKeyType, ek1);
                }
            }
            else
            {
        ...
        case ke2:
            if(appGui_Get_MenuState_IsOSD_Open())
            {
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE)
                {
                    appGui_SendKeyEvent(ucKeyType, ek2);
                }
            }
            else
            {
                LOG_MSG(db_APP_INPUTKEY, "\r\n ke2 : Lens Walk around "); //A70LV_John_0038 fix OE JIG bug
                halMotorLensSet(eLENS_CMDS_MOVING);
            }
            break;
        ...
        case ke3:
            if(appGui_Get_MenuState_IsOSD_Open())
            {
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE)
                {
                    appGui_SendKeyEvent(ucKeyType, ek3);
                }
            }
            else
            {
                ...
                if(m_cTestPattern == eCM_TEST_PATTERN_OFF)
                {
        ...
        case ke4: //A70LV_Larry_0256
            if(appGui_Get_MenuState_IsOSD_Open())
            {
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE)
                {
                    appGui_SendKeyEvent(ucKeyType, ek4);
                }
            }
            else
            {
        ...
        case ke5:
            if(appGui_Get_MenuState_IsOSD_Open())
            {
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE)
                {
                    appGui_SendKeyEvent(ucKeyType, ek5);
                }
            }
            else
            {
        ...
        case ke6: //A70LV_Larry_0256
            if(appGui_Get_MenuState_IsOSD_Open())
            {
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE)
                {
                    appGui_SendKeyEvent(ucKeyType, ek6);
                }
            }
            else
            {
        ...
        case ke7:
            if(appGui_Get_MenuState_IsOSD_Open())
            {
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE)
                {
                    appGui_SendKeyEvent(ucKeyType, ek7);
                }
            }
            else
            {
        ...
        case ke8:
            ...
            if(appGui_Get_MenuState_IsOSD_Open()) //T100_Sander_0016
            {
                ...
                {
                    appGui_SendKeyEvent(ucKeyType, ek8);
                }
            }
            else //LH OEJIG
            {
        ...
        case ke9:
            ...
            if(appGui_Get_MenuState_IsOSD_Open()) //T100_Sander_0016
            {
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
                {
                    appGui_SendKeyEvent(ucKeyType, ek9);
                }
            }
            else //LH OEJIG
            {
        ...
        case ke0:
            ...
            if(appGui_Get_MenuState_IsOSD_Open()) //T100_Sander_0016
            {
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
                {
                    appGui_SendKeyEvent(ucKeyType, ek0);
                }
            }
            else //LH OEJIG
            {
        ...
        case keMENU:
            ...
            if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
            {
    			...
    			{
                	appGui_SendKeyEvent(ucKeyType, ekMENU);
    			}

                if(m_cTestPattern != eCM_TEST_PATTERN_OFF) //A70LV_Larry_0296 //A70LV_Larry_0353
                {
                    UINT8 uTP = eCM_TEST_PATTERN_OFF;
        ...
        case keUP:
            ...
            if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
            {
                ...
                //appDataMgr_Data_Range_Get(edcSERVICE_CODE_INFO2, edrMAX, &ucMAX_Number);
                ...
                #endif
                {
    				...
    				{
                    	appGui_SendKeyEvent(ucKeyType, ekUP);
    				}
                }
        ...
        case keDOWN:
        {
            ...
            if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
            {
                ...
                //appDataMgr_Data_Range_Get(edcSERVICE_CODE_INFO2, edrMAX, &ucMAX_Number);
                {
                	...
                	{
                    	appGui_SendKeyEvent(ucKeyType, ekDOWN);
                	}
                }
                //G100_Wilsonj_0067 End
            }
        ...
        case keLEFT:
            ...
            if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
            {
                ...
                //appDataMgr_Data_Range_Get(edcSERVICE_CODE_INFO2, edrMAX, &ucMAX_Number);
                {
                	...
                	{
                    	appGui_SendKeyEvent(ucKeyType, ekLEFT);
                	}
                }
                //G100_Wilsonj_0067 End
            }
        ...
        case keRIGHT:
            ...
            if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
            {
                ...
                //appDataMgr_Data_Range_Get(edcSERVICE_CODE_INFO2, edrMAX, &ucMAX_Number);
                {
                	...
                	{
                    	appGui_SendKeyEvent(ucKeyType, ekRIGHT);
                	}
                }
                //G100_Wilsonj_0067 End
            }
        ...
        case keENTER:
            ...
            if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
            {
    			...
    			{
                	appGui_SendKeyEvent(ucKeyType, ekENTER);
    			}
            }
            break;
        case keBACK:
            ...
            if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
            {
            	//if(palInputProc_ADV_WarpBackCheck() == FALSE)		//G100_Doulas_0027 Modify
    			{
                	appGui_SendKeyEvent(ucKeyType, ekEXIT);
    			}

                if(m_cTestPattern != eCM_TEST_PATTERN_OFF) //A70LV_Larry_0296
                {
                    m_cTestPattern = eCM_TEST_PATTERN_OFF;
                    palDataMgr_Data_Access(edcSERVICE_TEST_PATTERN, edaWRITE_RAM_ONLY_NO_ACTION, &m_cTestPattern);
                }
            }
            break;
        /* Menu - end */
        ...
        case keAUTO:
            ...
            if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
            {
                appGui_SendKeyEvent(ucKeyType, ekAUTO);
            }
            break;
        ...
        case keINPUT:
            {
                ...
                if(palDataMgr_PIN_Protect_Checking() != ePASSWORD_PROTECT_UNLOCK)   //A70LV_Doulas_0222 Add
                {
                    break;
                }
    			...
    			if(utilWarp_GetOsdPatternType() != PAT_TYPE__OFF)		//A35G2_Owen_0002 //A65_OPTOMA_Doulas_0211
    			{
    				if(palInputProc_KeyTimerHander(&sInputKey) == TRUE)
            		{
                		appGui_SendKeyEvent(ucKeyType, ekINPUT);
            		}
                ...
                #ifdef CUSTOM_CHRISTIE
                {
                    ...
                    {
                        appDataMgr_MainInputSourceChangeToNext();
                        appGui_SendUpdateOSDEvent();        //A70LV_Doulas_0352 Add
                    }
                }
                #else
                appDataMgr_MainInputSourceChangeToNext();
        ...
        case keHOLD_LEFT:
            ...
            {
                appGui_SendKeyEvent(ucKeyType, ekLEFT_HOLD);
            }
            break;
        ...
        case keHOLD_RIGHT:
            if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0083
            {
                appGui_SendKeyEvent(ucKeyType, ekRIGHT_HOLD);
            }
            break;
    	//G100_Clare_0006, add, >>>
    	case kePATTERN:
    		LOG_MSG(db_APP_INPUTKEY, "\r\nPATTERN Key");
    		appGui_SendKeyEvent(ucKeyType, ekPATTERN);
    		break;
        case keMUTE:
            {
                UINT8 ucData = 0 ;
        ...
        case keRELEASE_BRIGHT:
    		{
                ...
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
                {
                    appGui_SendKeyEvent(ucKeyType, keRELEASE_BRIGHT); //A70LV_Larry_0073
                }
    		}
            break;
        ...
        case keRELEASE_PIP_SIZE:
    		{
                ...
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
                {
                    appGui_SendKeyEvent(ucKeyType, keRELEASE_PIP_SIZE); //A70LV_Larry_0073
                }
    		}
            break;
        ...
        case keRELEASE_CONTRAST:
    		{
                ...
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
                {
                    appGui_SendKeyEvent(ucKeyType, keRELEASE_CONTRAST); //A70LV_Larry_0073
                }
    		}
            break;
        ...
        case keRELEASE_PIP_LAYOUT:
    		{
                ...
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
                {
                    appGui_SendKeyEvent(ucKeyType, keRELEASE_PIP_LAYOUT); //A70LV_Larry_0073
                }
    		}
            break;
...
#if defined (PLATFORM_R70K)             //HICC2_Tim_0004, add, ***
// ==============================================================================
// FUNCTION NAME:
// DESCRIPTION: Handling the OE JIG remote key behavior for R70K ProAV.
//
// Params: psInputKey: key code
//
// Returns: OE JIG Command Type,
//
// modification history
// --------------------
...
// --------------------
// ==============================================================================
eOE_JIG_CMD_TYPE palInputProc_OEJig_HotKey(sKEY_DATA *psInputKey)
{
    eOE_JIG_CMD_TYPE eOE_JIG_Ret = eOE_JIG_CMD_TYPE_NUMBERS;
    sKEY_DATA sInputKey = {0};
    ...
    LOG_MSG(db_APP_INPUTKEY, "palInputProc_OEJig_HotKey(), Input Key: 0x02%X\r\n", sInputKey.wKeyCode);

    switch(sInputKey.wKeyCode)
    {
        //OE JIG special function ***
        case kePOWERON:
        {
            UINT8 ucData;

            LOG_MSG(db_APP_INPUTKEY, "R70K OE_JIG, Power On\r\n");

            eOE_JIG_Ret = eOE_JIG_CMD_TYPE_PF;
        ...
        case keSTANDBY:
        {
            //shut down without confirming.
            eOE_JIG_Ret = eOE_JIG_CMD_TYPE_PF;

            LOG_MSG(db_APP_INPUTKEY, "R70K OE_JIG, Power Off\r\n");
            palSystem_PowerDown();
        }
        break;
        ...
        case keGAMMA:
        {
            ...
            if( ucOE_JIG_PWM_Stage < ( ucOE_JIG_PWM_RANK - 1 ) )
            {
                ucOE_JIG_PWM_Stage ++;
            }
            else
            {
                ucOE_JIG_PWM_Stage = 0;
            }

            palDataMgr_Data_Access(edcFACTORY_FORMATTER_BLD_PWM_R, edaWRITE_RAM_ONLY_WITH_ACTION, &uiOE_JIG_PWM_TABLE[ucOE_JIG_PWM_Stage][0]);
            palDataMgr_Data_Access(edcFACTORY_FORMATTER_BLD_PWM_G, edaWRITE_RAM_ONLY_WITH_ACTION, &uiOE_JIG_PWM_TABLE[ucOE_JIG_PWM_Stage][1]);
            palDataMgr_Data_Access(edcFACTORY_FORMATTER_BLD_PWM_B, edaWRITE_RAM_ONLY_WITH_ACTION, &uiOE_JIG_PWM_TABLE[ucOE_JIG_PWM_Stage][2]);
            palDataMgr_Data_Access(edcFACTORY_FORMATTER_BLD_PWM_Y, edaWRITE_RAM_ONLY_WITH_ACTION, &uiOE_JIG_PWM_TABLE[ucOE_JIG_PWM_Stage][3]);
        }
        break;
        ...
        {
            eOE_JIG_Ret = eOE_JIG_CMD_TYPE_PF;
            uPMode = !uPMode; //HICC2_AC_0014
            palDataMgr_Data_Access(edcACTUATOR_SWITCH, edaWRITE_RAM_ONLY_WITH_ACTION, &uPMode);
            LOG_MSG(db_APP_INPUTKEY, "R70K OE_JIG, Acturator %s\r\n", (uPMode == 1) ? "On" : "Off" );
        }
        break;

        case keBRIGHT:              //PW Wheel increase
        {
            UINT16 ucPW_Index = PW_INDEX_MAX_VALUE;
            INT32 ilBourndary = PW_INDEX_MAX_VALUE;

            eOE_JIG_Ret = eOE_JIG_CMD_TYPE_PF;

            palDataMgr_Data_Access(edcPHOSPHOR_WHEEL_INDEX, edaREAD, &ucPW_Index);
            palDataMgr_Data_Range_Get(edcPHOSPHOR_WHEEL_INDEX, edrMAX, &ilBourndary);
            LOG_MSG(db_APP_INPUTKEY, "R70K OE_JIG, PW Index Current:%d, Max:%d\r\n", ucPW_Index, ilBourndary );

            if( ucPW_Index < ( (UINT16)ilBourndary ) )
            {
                ucPW_Index++;
            }
            else
            ...
            palDataMgr_Data_Access(edcPHOSPHOR_WHEEL_INDEX, edaWRITE_THROUGH_WITH_ACTION, &ucPW_Index);
        }
        break;

        case kePIP_SIZE:            //PW Wheel decrease
        {
            UINT16 ucPW_Index = PW_INDEX_MIN_VALUE;
            INT32 ilBourndary = PW_INDEX_MIN_VALUE;

            eOE_JIG_Ret = eOE_JIG_CMD_TYPE_PF;

            palDataMgr_Data_Access(edcPHOSPHOR_WHEEL_INDEX, edaREAD, &ucPW_Index);
            palDataMgr_Data_Range_Get(edcPHOSPHOR_WHEEL_INDEX, edrMIN, &ilBourndary);
            LOG_MSG(db_APP_INPUTKEY, "R70K OE_JIG, PW Index Current:%d, Min:%d\r\n", ucPW_Index, ilBourndary );
        ...
        case keCONTRAST:                //FW Wheel increase
        {
            UINT16 ucFW_Index = FW_INDEX_MAX_VALUE;
            INT32 ilBourndary = FW_INDEX_MAX_VALUE;

            eOE_JIG_Ret = eOE_JIG_CMD_TYPE_PF;

            palDataMgr_Data_Access(edcFILTER_WHEEL_INDEX, edaREAD, &ucFW_Index);
            palDataMgr_Data_Range_Get(edcFILTER_WHEEL_INDEX, edrMAX, &ilBourndary);
            LOG_MSG(db_APP_INPUTKEY, "R70K OE_JIG, FW Index Current:%d, Max:%d\r\n", ucFW_Index, ilBourndary );

            if( ucFW_Index < ( (UINT16)ilBourndary ) )
            {
                ucFW_Index++;
            }
            else
            ...
            palDataMgr_Data_Access(edcFILTER_WHEEL_INDEX, edaWRITE_THROUGH_WITH_ACTION, &ucFW_Index);
        }
        break;

        case kePIP_LAYOUT:              //FW Wheel decrease
        {
            UINT16 ucFW_Index = FW_INDEX_MIN_VALUE;
            INT32 ilBourndary = FW_INDEX_MIN_VALUE;
            LOG_MSG(db_APP_INPUTKEY, "R70K OE_JIG, FW Index Current:%d, Min:%d\r\n", ucFW_Index, ilBourndary );

            eOE_JIG_Ret = eOE_JIG_CMD_TYPE_PF;

            palDataMgr_Data_Access(edcFILTER_WHEEL_INDEX, edaREAD, &ucFW_Index);
            palDataMgr_Data_Range_Get(edcFILTER_WHEEL_INDEX, edrMIN, &ilBourndary);
        ...
        case ke1:                       //LD 1 On/Off
        case ke2:                       //LD 2 On/Off
        //case ke3:                       //LD 3 On/Off, no such LD
        //case ke4:                       //LD 4 On/Off, no such LD
        case ke5:                       //LD 5 On/Off
        case ke6:                       //LD 6 On/Off
        {
            BOOL bLD_Enable;
            UINT8 ucLD_No = sInputKey.wKeyCode - ke1;
            eDATA_CODE eData_Code = edcLD_ENALE_01 + ucLD_No;

            eOE_JIG_Ret = eOE_JIG_CMD_TYPE_PF;
            palDataMgr_Data_Access(eData_Code, edaREAD, &bLD_Enable);
            ...
            LOG_MSG(db_APP_INPUTKEY, "R70K OE_JIG, LD %d %s\r\n", (ucLD_No + 1), ( ( bLD_Enable == TRUE ) ? "On":"Off" ) );
            palDataMgr_Data_Access(eData_Code, edaWRITE_THROUGH_WITH_ACTION, &bLD_Enable);
        }
        break;

        case keTEST:
        {
            UINT8 ucFactory_TestPattern;

            eOE_JIG_Ret = eOE_JIG_CMD_TYPE_PF;
            palDataMgr_Data_Access(edcFACTORY_TEST_PATTERN, edaREAD, &ucFactory_TestPattern);

            if( ucFactory_TestPattern < ( eCM_FACTORY_TP_NUMBER - 1) )
            {
                ucFactory_TestPattern++;
            }
            else
            {
                ucFactory_TestPattern = 0;
            }

            LOG_MSG(db_APP_INPUTKEY, "R70K OE_JIG, Factory TP: %d\r\n", ucFactory_TestPattern);
        ...
        case keSWAP:
        {
            //it is executed in iScalerH2CT
             eOE_JIG_Ret = eOE_JIG_CMD_TYPE_CT;
             palInputProc_KeyEventSend(sInputKey);
             LOG_MSG(db_APP_INPUTKEY, "R70K OE_JIG, CT OE JIG key: %d\r\n", sInputKey.wKeyCode );
        }
        break;
        ...
        case keMENU:
            if(palInputProc_KeyTimerHander(&sInputKey) == TRUE)
            {
            	if(!palInputProc_DDP_PatternCheck())
				{
                    palInputProc_KeyEventSend(sInputKey);
				}
        ...
        case keHOLD_ZOOMIN:
            LOG_MSG(db_APP_INPUTKEY, "\r\n OE JIG, keHOLD_ZOOMIN");
            palInputProc_KeyEventSend(sInputKey);
            palMotorZoomSet(eZOOMLENS_CMD_ZOOM_IN_RUN);
            break;

        case keHOLD_ZOOMOUT:
            LOG_MSG(db_APP_INPUTKEY, "\r\n OE JIG, keHOLD_ZOOMOUT");
            palInputProc_KeyEventSend(sInputKey);
            palMotorZoomSet(eZOOMLENS_CMD_ZOOM_OUT_RUN);
            break;

        case keHOLD_FOCUSUP:
            LOG_MSG(db_APP_INPUTKEY, "\r\n OE JIG, keHOLD_FOCUSUP");
            palInputProc_KeyEventSend(sInputKey);
            palMotorFocusSet(eZOOMLENS_CMD_FOCUS_NEAR_RUN);
            break;

        case keHOLD_FOCUSDOWN:
            LOG_MSG(db_APP_INPUTKEY, "\r\n OE JIG, keHOLD_FOCUSDOWN");
            palInputProc_KeyEventSend(sInputKey);
            palMotorFocusSet(eZOOMLENS_CMD_FOCUS_FAR_RUN);
            break;

        case keHOLD_LENSHLEFT:
            LOG_MSG(db_APP_INPUTKEY, "\r\n OE JIG, keHOLD_LENSHLEFT");
            palInputProc_KeyEventSend(sInputKey);
            palMotorLensSet(eLENS_CMDS_RUN_LEFT);
            break;

        case keHOLD_LENSHRIGHT:
            LOG_MSG(db_APP_INPUTKEY, "\r\n OE JIG, keHOLD_LENSHRIGHT");
            palInputProc_KeyEventSend(sInputKey);
            palMotorLensSet(eLENS_CMDS_RUN_RIGHT);
            break;

        case keHOLD_LENSVUP:
            LOG_MSG(db_APP_INPUTKEY, "\r\n OE JIG, keHOLD_LENSVUP");
            palInputProc_KeyEventSend(sInputKey);
            palMotorLensSet(eLENS_CMDS_RUN_UP);
            break;

        case keHOLD_LENSVDOWN:
            LOG_MSG(db_APP_INPUTKEY, "\r\n OE JIG, keHOLD_LENSVDOWN");
            palInputProc_KeyEventSend(sInputKey);
            palMotorLensSet(eLENS_CMDS_RUN_DOWN);
            break;
        //Lens &&&
        ...
        case keINPUT:
            {
                ...
                if(palDataMgr_PIN_Protect_Checking() != ePASSWORD_PROTECT_UNLOCK) //(appGui_PIN_Protect_Checking() == eEXEC_CODE_FAIL)   //A70LV_Doulas_0222 Add
                {
                    break;
                }
            ...
            #ifdef CUSTOM_CHRISTIE
                {
                    if(InputKey == eSOURCE_KEY_LIST_ALL_SOURCE)
                    {
                        UINT16 uiInput = 0;
        ...
        {
           eOE_JIG_Ret = eOE_JIG_CMD_TYPE_NOP;
        }
           break;

    }

    LOG_MSG(db_APP_INPUTKEY, "palInputProc_OEJig_HotKey(), PF, %s OE JIG Key: 0x02%X\r\n", ((eOE_JIG_Ret == TRUE) ? "is":"isn't"));
...
BOOL palInputProc_OEJig_HotKey(sKEY_DATA *psInputKey)
{
    ...
    switch(sInputKey.wKeyCode)
    {
        ...
        case ke1:
            {
                BOOL bLD_Enable;
                palDataMgr_Data_Access(edcLD_ENALE_01, edaREAD, &bLD_Enable);
                if(bLD_Enable == TRUE)
                {
                    bLD_Enable = FALSE;
                    palDataMgr_Data_Access(edcLD_ENALE_01, edaWRITE_THROUGH_WITH_ACTION, &bLD_Enable);
                    LOG_MSG(db_APP_INPUTKEY, "\r\n LD 1 Off\r\n");
                }
                else
                {
        ...
        case ke2:
            {
                BOOL bLD_Enable;
                palDataMgr_Data_Access(edcLD_ENALE_02, edaREAD, &bLD_Enable);
                if(bLD_Enable == TRUE)
                {
                    bLD_Enable = FALSE;
                    palDataMgr_Data_Access(edcLD_ENALE_02, edaWRITE_THROUGH_WITH_ACTION, &bLD_Enable);
                    LOG_MSG(db_APP_INPUTKEY, "\r\n LD 2 Off\r\n");
                }
                else
                {
        ...
        case ke3:
            {
                BOOL bLD_Enable;
                palDataMgr_Data_Access(edcLD_ENALE_03, edaREAD, &bLD_Enable);
                if(bLD_Enable == TRUE)
                {
                    bLD_Enable = FALSE;
                    palDataMgr_Data_Access(edcLD_ENALE_03, edaWRITE_THROUGH_WITH_ACTION, &bLD_Enable);
                    LOG_MSG(db_APP_INPUTKEY, "\r\n LD 3 Off\r\n");
                }
                else
                {
        ...
        case ke4: //A70LV_Larry_0256
            {
                BOOL bLD_Enable;
                palDataMgr_Data_Access(edcLD_ENALE_04, edaREAD, &bLD_Enable);
                if(bLD_Enable == TRUE)
                {
                    bLD_Enable = FALSE;
                    palDataMgr_Data_Access(edcLD_ENALE_04, edaWRITE_THROUGH_WITH_ACTION, &bLD_Enable);
                    LOG_MSG(db_APP_INPUTKEY, "\r\n LD 4 Off\r\n");
                }
                else
                {
        ...
        case ke5:
			{
                BOOL bLD_Enable;
                palDataMgr_Data_Access(edcLD_ENALE_05, edaREAD, &bLD_Enable);
                if(bLD_Enable == TRUE)
                {
                    bLD_Enable = FALSE;
                    palDataMgr_Data_Access(edcLD_ENALE_05, edaWRITE_THROUGH_WITH_ACTION, &bLD_Enable);
                    LOG_MSG(db_APP_INPUTKEY, "\r\n LD 5 Off\r\n");
                }
                else
                {
        ...
        case ke6: //A70LV_Larry_0256
        	{
                BOOL bLD_Enable;
                palDataMgr_Data_Access(edcLD_ENALE_06, edaREAD, &bLD_Enable);
                if(bLD_Enable == TRUE)
                {
                    bLD_Enable = FALSE;
                    palDataMgr_Data_Access(edcLD_ENALE_06, edaWRITE_THROUGH_WITH_ACTION, &bLD_Enable);
                    LOG_MSG(db_APP_INPUTKEY, "\r\n LD 6 Off\r\n");
                }
                else
                {
        ...
        case ke8:
            ...
            palInputProc_KeyEventSend(sInputKey); //HICC2_Steven_0010
            #else
            if(IsOSDOpen && !appGui_Is_NotSuported_Menu())
            {
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
                {
                    palInputProc_KeyEventSend(sInputKey);
        ...
        case ke0:
			LOG_MSG(db_APP_INPUTKEY, "\r\n ke0 : Lens walk around ");
            palMotorLensSet(eLENS_CMDS_MOVING); //HICC2_Doulas_0105
            break;
		...
		case keHKEYSTONE_RIGHT:
            ...
            else
            {
                ...
                ucTP = eCM_FACTORY_TP_FULL_SREEN_W;//eCM_FACTORY_TP_WRGB64;//eFACTORY_TP_FULL_SREEN_W; //HICC2_Doulas_0125//HICC2_Doulas_0032 //HICC2_Casper_0022
                palDataMgr_Data_Access(edcFACTORY_TEST_PATTERN, edaWRITE_THROUGH_WITH_ACTION, &ucTP);
        	}
			break;
        ...
        case keAUTO:
            ...
            {
                UINT8 ucValue = 0;
                if(rcSUCCESS == halFormatter_XPR_CalibrationPattern_Get(&ucValue))
                {
                    if(ucValue)
                    {
        ...
        case keINPUT:
            {
                ...
                #ifdef CUSTOM_CHRISTIE
                {
                    ...
                    {
                        palDataMgr_MainInputSourceChangeToNext();
                        palDataMgr_UI_EventSend(edcUI_EVENT_UPDATE_OSD, TRUE, NULL); //appGui_SendUpdateOSDEvent();        //A70LV_Doulas_0352 Add
                    }
                }
                #else
                palDataMgr_MainInputSourceChangeToNext();
        ...
        case keBRIGHT: //HICC2_Doulas_0096
        case keHOLD_BRIGHT:
        	{
	            UINT16 ucPW_Index = PW_INDEX_MAX_VALUE;
	            INT32 ilBourndary = PW_INDEX_MAX_VALUE;
	            ...
	            palDataMgr_Data_Access(edcPHOSPHOR_WHEEL_INDEX, edaREAD, &ucPW_Index);
	            palDataMgr_Data_Range_Get(edcPHOSPHOR_WHEEL_INDEX, edrMAX, &ilBourndary);
	            LOG_MSG(db_APP_INPUTKEY, "OE_JIG, PW Index Current:%d, Max:%d\r\n", ucPW_Index, ilBourndary );
        ...
        case keHOLD_PIP_SIZE:
            {
	            ...
	            palDataMgr_Data_Access(edcPHOSPHOR_WHEEL_INDEX, edaREAD, &ucPW_Index);
	            palDataMgr_Data_Range_Get(edcPHOSPHOR_WHEEL_INDEX, edrMIN, &ilBourndary);
	            LOG_MSG(db_APP_INPUTKEY, "OE_JIG, PW Index Current:%d, Min:%d\r\n", ucPW_Index, ilBourndary );
        ...
        case keCONTRAST: //HICC2_Doulas_0096
        case keHOLD_CONTRAST:
        	{
	            UINT16 ucFW_Index = FW_INDEX_MAX_VALUE;
	            INT32 ilBourndary = FW_INDEX_MAX_VALUE;
	            ...
	            palDataMgr_Data_Access(edcFILTER_WHEEL_INDEX, edaREAD, &ucFW_Index);
	            palDataMgr_Data_Range_Get(edcFILTER_WHEEL_INDEX, edrMAX, &ilBourndary);
	            LOG_MSG(db_APP_INPUTKEY, "OE_JIG, FW Index Current:%d, Max:%d\r\n", ucFW_Index, ilBourndary );
        ...
        case kePIP_LAYOUT://FW index increase
        case keHOLD_PIP_LAYOUT:
            {
	            UINT16 ucFW_Index = FW_INDEX_MIN_VALUE;
	            INT32 ilBourndary = FW_INDEX_MIN_VALUE;
        ...
        case keGAMMA:   //HICC2_Doulas_0096
        	if(IsOSDOpen)
            {
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
                {
            ...
            {
            	static UINT8 ucPowerMode = eCM_POWER_MODE_30;
				if(ucPowerMode == eCM_POWER_MODE_30)
					ucPowerMode = eCM_POWER_MODE_50;
				else if(ucPowerMode == eCM_POWER_MODE_50)
					ucPowerMode = eCM_POWER_MODE_100;
				else if(ucPowerMode == eCM_POWER_MODE_100)
					ucPowerMode = eCM_POWER_MODE_30;
                palDataMgr_LightSource_Set(ucPowerMode, 100); //HICC2_Doulas_0096//12 is eCM_POWER_MODE_30
            	//halFormatter_PowerMode_Set(4);
				//halFormatter_SystemModeSet(12);
            }
            break;

        case kePIP:	// PW index increase
        //case keHOLD_PIP:
			{
			    if(uPMode == 0)
                {
                    uPMode = 1;
                    halFormatter_Actuator_Switch(0);
                    LOG_MSG(db_APP_INPUTKEY, "\r\n Actuator Off");
                }
                else
                {
                    uPMode = 0;
                    halFormatter_Actuator_Switch(1);
        		    LOG_MSG(db_APP_INPUTKEY, "\r\n Actuator On");
                }
			}
            break;
        ...
        case keTEST:
			...
			#if 1
			if(ucFactory_TestPattern == eCM_FACTORY_TP_ACTUATOR)
			{
				ucFactory_TestPattern = 0;
			}
			else
        ...
        case keOSD:
			...
			#if 1
			if(ucFactory_TestPattern == 0)
			{
				ucFactory_TestPattern = eCM_FACTORY_TP_ACTUATOR;
			}
			else
			{
				ucFactory_TestPattern--;
...
#else // defined (PLATFORM_R70K)        //HICC2_Tim_0004, add, &&&
...
BOOL palInputProc_OEJig_HotKey(sKEY_DATA *psInputKey)
{
    ...
    switch(sInputKey.wKeyCode)
    {
        ...
        case ke1:
            ...
            {
                palDataMgr_LightSource_Set(eCM_POWER_MODE_30, 100); //HICC2_Doulas_0096//12 is eCM_POWER_MODE_30
            	//halFormatter_PowerMode_Set(4);
				//halFormatter_SystemModeSet(12);
            }
            break;
        ...
        case ke2:
            ...
            {
                palDataMgr_LightSource_Set(eCM_POWER_MODE_50, 100);  //HICC2_Doulas_0096//11 is eCM_POWER_MODE_50
                //ucData = 1;
        ...
        case ke3:
            ...
            {
#ifndef OE_JIG_SPECIAL_PART
                palDataMgr_LightSource_Set(eCM_POWER_MODE_100, 100); //HICC2_Doulas_0096//13 is eCM_POWER_MODE_10
#endif /* OE_JIG_SPECIAL_PART */
            	//halFormatter_PowerMode_Set(4);
				//halFormatter_SystemModeSet(12);
            }
            break;
        ...
        case keINFO:
            LOG_MSG(db_APP_INPUTKEY, "\r\n keINFO : Lens Calibration "); //A70LV_John_0038 fix OE JIG bug
            palMotorLensSet(eLENS_CMDS_CALIBRAION); //HICC2_Doulas_0105
            break;
        /* Lens - end */
        ...
        case keBRIGHT: //HICC2_Doulas_0096
        #if defined(PLATFORM_H30_4K) //HICC2_AC_0006
            {
                BOOL bLD_Enable;
                palDataMgr_Data_Access(edcLD_ENALE_01, edaREAD, &bLD_Enable);
                if(bLD_Enable == TRUE)
...
#endif //defined (PLATFORM_R70K)        //HICC2_Tim_0004, add

#endif /* OE_JIG */

void palInputProc_PowerOnKeyHandle(UINT16 uiKey)
{
    if(ePOWER_STATE_STANDBY == palSystem_PowerStateGet())
    {
        if(kePOWERON == uiKey)
        {
            palSystem_PowerKey();
...
{
    sKEY_DATA sInputKey = {0};
	UINT8  ucQuickKeyValue = 0;		//A65_OPTOMA_Doulas_0025

    if(rcSUCCESS == (eRESULT)utilQueueRead(&m_sInputRingBuffer, (UINT8*)&sInputKey))
    {
		UINT8 ucKeyType = sInputKey.eKeyType;

        palInputProc_PowerOnKeyHandle(sInputKey.wKeyCode);

        if(m_cDisableKey || ((palSystem_PowerStateGet() != ePOWER_STATE_ACTIVE) && palSystem_SystemReadyGet())) //A65_Owen_0001
        {
            return INPUT_POLLING_PERIOD / POLL_PERIOD ;
        }

        if(utilCounterGet(eCOUNTER_TYPE_FACTORY_RESET) > 0) //A70LK_Jacky_0050
        {
            return INPUT_POLLING_PERIOD / POLL_PERIOD ;
        }

        appInput_Uart_Switch_Process((eKEY_LIST)sInputKey.wKeyCode);

        if(palIllumination_BurnIn_Enable()) //A70LV_Larry_0016 //A70LV_Larry_0111
        {
			...
			else
			{
				...
				palDataMgr_Data_Access(edcBURNIN_ENABLE, edaWRITE_THROUGH_WITH_ACTION, &ucValue);
				palDataMgr_Data_Access(edcBURN_IN, edaWRITE_THROUGH_WITH_ACTION, &ucValue);
			}
#endif
        }
        else //A70LV_Larry_0110
        {
            appKeypadBacklighthandle(&sInputKey);
            ...
            else if(palInputProc_ServiceCodeCheck(&sInputKey) == eEXEC_CODE_PASS) //HICC2_Steven_0043
            {
                return INPUT_POLLING_PERIOD / POLL_PERIOD;
            }
        }
        ...
        if(CFG_CUSTOMER_ID == CUSTOMER_ID_OPTOMA)
        {
            ...
            {
                palEnvironment_SleepTimerClear();   //A70LV_Larry_0082 //A70LV_Larry_0130
            }
        }
        else
        {
            if(palEnvironment_SleepTimerGet() <= 60 && sInputKey.wKeyCode == keBACK)  //G100_Simon_0050
            {
                palEnvironment_SleepTimerClear();   //A70LV_Larry_0082 //A70LV_Larry_0130
            }
        }
        ...
        LOG_MSG(db_APP_INPUTKEY, "\r\n KeyCode = %d\r\n",sInputKey.wKeyCode); //A70LV_John_0038 fix OE JIG bug

    #if defined (PLATFORM_R70K)         //HICC2_Tim_0004, add, ***

        {
           eOE_JIG_CMD_TYPE eType = palInputProc_OEJig_HotKey(&sInputKey);
            if( eType == eOE_JIG_CMD_TYPE_PF )
            {
                LOG_MSG(db_APP_INPUTKEY, "eOE_JIG_CMD_TYPE_PF\r\n");
            }
            else if( eType == eOE_JIG_CMD_TYPE_CT )
            {
                LOG_MSG(db_APP_INPUTKEY, "eOE_JIG_CMD_TYPE_CT\r\n");
            }
        }
...
#else
    	if( TRUE == palInputProc_BIST_KeyCheckFun(sInputKey.wKeyCode)) //HICC2_Steven_0022
    	{
    		return INPUT_POLLING_PERIOD / POLL_PERIOD ;
    	}

        if((palInputProc_AdvWarpPatternOn_KeyCheck(sInputKey.wKeyCode) == TRUE) &&
           (utilCounterGet(eCOUNTER_TYPE_GUI_KEY) == 0))
        {
            palInputProc_KeyTimerHander(&sInputKey);
        ...
        switch(sInputKey.wKeyCode)
        {
            ...
            case kePOWERON:
                {
                    ...
                    else if(sInputKey.eKeyType == eKEYINPUT_KEYPAD) //A70LV_Larry_0023
                    {
                    	...
                    	sInputKey.wKeyCode = keSTANDBY; //HICC2_Steven_0015
                        palInputProc_KeyEventSend(sInputKey);
                    }
                }
                break;
            ...
            case keSTANDBY: //A70LV_Larry_0001
                {
                	UINT8 ucFactory_TestPattern = eCM_FACTORY_TP_NONE; 	//A70LK_Doulas_0018 //HICC2_Casper_0019 //HICC2_Casper_0022
                    LOG_MSG(db_APP_INPUTKEY, "\r\nStandby Key");
					...
					if(ucFactory_TestPattern != eCM_FACTORY_TP_NONE) 	//A70LK_Doulas_0018 //HICC2_Casper_0022
					{
            ...
            case keMENU:
                ...
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
                {
                    if(!palInputProc_DDP_PatternCheck()) //HICC2_Casper_0019
                    {
                        palInputProc_KeyEventSend(sInputKey);
                    }
                }
            ...
            case keUP:
                ...
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE) //A70LV_Larry_0074
                {
                    palDataPath_FirstLanguageCountSet(0);//H30K_David_0047
                    palInputProc_KeyEventSend(sInputKey);
                }
                break;
            ...
            case keZOOMIN:
                {
                    LOG_MSG(db_APP_INPUTKEY, "\r\n keZOOMIN");
                    if(((m_ucLensType == eLENS_ID_B20U100) || (m_ucLensType == eLENS_ID_NVRAM_B20U100)) && (m_ucFujiLensDetect == 1)) //HICC2_Julie_0071//HICC2_Julie_0072//H30K_Julie_0005
                    {
                        palDataMgr_Data_Access(edcFUJI_LENS_DETECT, edaWRITE_RAM_ONLY_WITH_ACTION, (void*)&m_ucFujiLensDetect);
                    }
                    else if((m_cLensMotorLock == eLENS_LOCK_ALL_ALLOW) && (eFUNC_CONTROL_ENABLE == palDataMgr_DataCode_Control(edcLENS_LOCK)))
                    {
                        palInputProc_KeyEventSend(sInputKey);

                        if((m_ucLensType == eLENS_ID_B20U100) || (m_ucLensType == eLENS_ID_NVRAM_B20U100)) //HICC2_Julie_0071
                        {
                            UINT8 ucBackFocus = 0xFF;
                            palDataMgr_Data_Access(edcBACK_FOCUS_SWITCH, edaREAD, &ucBackFocus);

                            if((sInputKey.eKeyType == eKEYINPUT_LAN) && (ucBackFocus != 0))
                            {
                                ucBackFocus = 0;
                                palDataMgr_Data_Access(edcBACK_FOCUS_SWITCH, edaWRITE_THROUGH_WITH_ACTION, &ucBackFocus);
                            }
                        }
                    ...
                    else if((m_cLensMotorLock == eLENS_LOCK_ALL_ALLOW) && (eFUNC_CONTROL_ENABLE == palDataMgr_DataCode_Control(edcLENS_LOCK)))
                    {
                        ...
                        if((m_ucLensType == eLENS_ID_B20U100) || (m_ucLensType == eLENS_ID_NVRAM_B20U100)) //HICC2_Julie_0071
                        {
                            ...
                            if((sInputKey.eKeyType == eKEYINPUT_LAN) && (ucBackFocus != 0))
                            {
                                ucBackFocus = 0;
                                palDataMgr_Data_Access(edcBACK_FOCUS_SWITCH, edaWRITE_THROUGH_WITH_ACTION, &ucBackFocus);
                                palDataMgr_UI_EventSend(edcUI_EVENT_BACKFOCUS_MENU, ucBackFocus, NULL);
                            }
                        }
                        else if(m_bMotorFocusEnale) //A70LV_Larry_0219
                        {
							palMotorFocusSet(eZOOMLENS_CMD_FOCUS_FAR_STEP);
                        }
                    }
            ...
            case keBACK_FOCUSUP:
                {
                    LOG_MSG(db_APP_INPUTKEY, "\r\n keBACK_FOCUSUP Key");
                    if((m_cLensMotorLock == eLENS_LOCK_ALL_ALLOW) && (eFUNC_CONTROL_ENABLE == palDataMgr_DataCode_Control(edcLENS_LOCK)))
                    {
                        if((m_ucLensType == eLENS_ID_B20U100) || (m_ucLensType == eLENS_ID_NVRAM_B20U100))//HICC2_Julie_0072//H30K_Julie_0005
                        {
                            UINT8 ucBackFocus = 0xFF;
                            palDataMgr_Data_Access(edcBACK_FOCUS_SWITCH, edaREAD, &ucBackFocus);
            ...
            case keBACK_FOCUSDOWN:
                {
                    LOG_MSG(db_APP_INPUTKEY, "\r\n keBACK_FOCUSDOWN Key");
                    if((m_cLensMotorLock == eLENS_LOCK_ALL_ALLOW) && (eFUNC_CONTROL_ENABLE == palDataMgr_DataCode_Control(edcLENS_LOCK)))
                    {
                        if((m_ucLensType == eLENS_ID_B20U100) || (m_ucLensType == eLENS_ID_NVRAM_B20U100))//HICC2_Julie_0072//H30K_Julie_0005
                        {
            ...
            case keOSD: //A70LV_Larry_0111 //A70LV_Larry_0248 //HICC2_Julie_0033
                {
                    LOG_MSG(db_APP_INPUTKEY, "\r\nOSD Key");
            ...
            case keFOCUS:
                {
                    LOG_MSG(db_APP_INPUTKEY, "\r\nFocus Key");
                    if(((m_ucLensType == eLENS_ID_B20U100) || (m_ucLensType == eLENS_ID_NVRAM_B20U100)) && (m_ucFujiLensDetect == 1))
                    {
                        palDataMgr_Data_Access(edcFUJI_LENS_DETECT, edaWRITE_RAM_ONLY_WITH_ACTION, (void*)&m_ucFujiLensDetect);
                        break;
                    }
            ...
            case keINPUT:   //A70LV_Doulas_0082 Modify //A70LV_Larry_0248
                {
                    ...
                    #if defined(CUSTOM_CHRISTIE)
                    {
                        if(InputKey == eSOURCE_KEY_LIST_ALL_SOURCE) //A70LV_Larry_0109
                        {
                            ...
                            palDataMgr_UI_EventSend(edcUI_EVENT_LIST_ALL_SOURCE, TRUE, NULL); //appGui_SendListAllSourceEvent();
                        }
                        else
                        {
                            palDataMgr_MainInputSourceChangeToNext();
                    ...
                    {
                     	UINT16 uiInput = palDataPath_LastInputDetectGet();
                        if(InputKey == eSOURCE_KEY_CHANGE_SOURCE) //A70LV_Larry_0109
                        {
                            //UINT16 uiInput = palDataPath_LastInputDetectGet();
                        ...
                        {
                            if( palDataPath_IsSourceLock() == TRUE)//H30K_David_0051
                            {
                              	palDataMgr_Data_Access(edcSOURCE_INFO, edaWRITE_RAM_ONLY_NO_ACTION, &uiInput);
                            	palDataMgr_UI_EventSend(edcUI_EVENT_LIST_ALL_SOURCE, TRUE, NULL); //appGui_SendListAllSourceEvent();
            ...
            case keHKEYSTONE_LEFT: //A70LV_Larry_0148
                {
                	...
                	if(palDataMgr_DataCode_Control(edcHORIZONTAL_KEYSTONE) != eFUNC_CONTROL_ENABLE)  //H2PF_Simon_0132
                	{
		                palDataMgr_UI_EventSend(edcUI_EVENT_SOURCE_MESSAGE, eSOURCE_MESSAGE_NOT_SUPPORTED, NULL); //appGui_SendSourceMessage(eSOURCE_MESSAGE_NOT_SUPPORTED);
                	}
                	else
                	{
                        palInputProc_KeyEventSend(sInputKey);
            ...
            case keVKEYSTONE_UP: //A70LV_Larry_0148
                {
                    ...
                    //if( (palGeo_ApLinkFlag_Get() == TRUE ) ||				//A70Gen2_Steven_0005 fixed ISS-0008137 // R70CDS_Bruce#0087
                    //	(palDataMgr_CurrentWarpMemoryType() == WARPING_TYPE_AP || palDataMgr_CurrentWarpMemoryType() == WARPING_TYPE_AP_MYSTIQUE))// R70CDS_Bruce#0084
                	if(palDataMgr_DataCode_Control(edcVERTICAL_KEYSTONE) != eFUNC_CONTROL_ENABLE)  //H2PF_Simon_0132
                    {
                    	palDataMgr_UI_EventSend(edcUI_EVENT_SOURCE_MESSAGE, eSOURCE_MESSAGE_NOT_SUPPORTED, NULL); //appGui_SendSourceMessage(eSOURCE_MESSAGE_NOT_SUPPORTED);
                    }
                    else
            ...
            case keHOLD_ZOOMIN:
                {
                    ...
                    else if((m_cLensMotorLock == eLENS_LOCK_ALL_ALLOW) && (eFUNC_CONTROL_ENABLE == palDataMgr_DataCode_Control(edcLENS_LOCK)))
                    {
                        ...
                        if((m_ucLensType == eLENS_ID_B20U100) || (m_ucLensType == eLENS_ID_NVRAM_B20U100)) //HICC2_Julie_0071//H30K_Julie_0005
                        {
                            UINT8 ucBackFocus = 0xFF;
                            palDataMgr_Data_Access(edcBACK_FOCUS_SWITCH, edaREAD, &ucBackFocus);
            ...
            case keHOLD_BACK_FOCUSUP:
                {
                    LOG_MSG(db_APP_INPUTKEY, "\r\n keHOLD_BACK_FOCUSUP Key");
            ...
            case keHOLD_BACK_FOCUSDOWN:
                {
                    LOG_MSG(db_APP_INPUTKEY, "\r\n keHOLD_BACK_FOCUSDOWN Key");
                    if(m_cLensMotorLock == eLENS_LOCK_ALL_ALLOW)
                    {
                        if((m_ucLensType == eLENS_ID_B20U100) || (m_ucLensType == eLENS_ID_NVRAM_B20U100))//HICC2_Julie_0072//H30K_Julie_0005
                        {
            ...
            case keHOLD_STANDBY:
                //appGui_SendKeyEvent(ucKeyType, ekSTANDBY); //A70LV_Larry_0333 mask
                break;
            ...
            case keTEST:    //A70LV_Doulas_0230 modify//A70LV_Doulas_0095
                if(palInputProc_KeyTimerHander(&sInputKey) == TRUE)
                {
					...
					{
						if(ucTP == eCM_FACTORY_TP_NONE)
							ucTP = eCM_FACTORY_TP_FULL_SREEN_W;
						else if(ucTP == eCM_FACTORY_TP_FULL_SREEN_W)
							ucTP = eCM_FACTORY_TP_13POINTS;
						else if(ucTP == eCM_FACTORY_TP_13POINTS)
							ucTP = eCM_FACTORY_TP_RED;
						else if(ucTP == eCM_FACTORY_TP_RED)
							ucTP = eCM_FACTORY_TP_GREEN;
						else if(ucTP == eCM_FACTORY_TP_GREEN)
							ucTP = eCM_FACTORY_TP_BLUE;
						else if(ucTP == eCM_FACTORY_TP_BLUE)
							ucTP = eCM_FACTORY_TP_BLACK;
						else
							ucTP = eCM_FACTORY_TP_NONE;

						palDataMgr_Data_Access(edcFACTORY_TEST_PATTERN, edaWRITE_THROUGH_WITH_ACTION, &ucTP) ;
					}
				#else
					if(CFG_CUSTOMER_ID == CUSTOMER_ID_OPTOMA || CFG_CUSTOMER_ID == CUSTOMER_ID_CHRISTIE)
					{
        ...
        appInput_Motor_Process((eKEY_LIST)sInputKey.wKeyCode); //A70LV_Larry_0219
#endif
    }
    ...
    //LSM UART
    if((utilCounterGet(eCOUNTER_TYPE_APP_LENS) == 0) && m_ucMotorLensAPPEnable) //HICC2_Casper_0030 //HICC2_Casper_0033 //HICC2_Casper_0041
    {
        sLENS_INFO sLensInfo = {0};
        palDataMgr_Data_Access(edcLENS_INFO, edaREAD, &sLensInfo);

        if(m_ucMotorLensAPPEnable & 0x01)
        {
            palDataMgr_Data_Access(edcCUR_H_POSITION, edaWRITE_RAM_ONLY_NO_ACTION, (void*)&sLensInfo.wLensHPosition);
            palDataMgr_Data_Access(edcCUR_H_DIR, edaWRITE_RAM_ONLY_NO_ACTION, (void*)&sLensInfo.cDirH);
        }

        if(m_ucMotorLensAPPEnable & 0x02)
        {
            palDataMgr_Data_Access(edcCUR_V_POSITION, edaWRITE_RAM_ONLY_NO_ACTION, (void*)&sLensInfo.wLensVPosition);
            palDataMgr_Data_Access(edcCUR_V_DIR, edaWRITE_RAM_ONLY_NO_ACTION, (void*)&sLensInfo.cDirV);
        }
        m_ucMotorLensAPPEnable = 0;

        LOG_MSG(db_APP_INPUTKEY, "Lens posiotion (%d,%d) Dir(%d,%d)\r\n",sLensInfo.wLensHPosition,sLensInfo.wLensVPosition,sLensInfo.cDirH,sLensInfo.cDirV);
    }
#else
    if((utilCounterGet(eCOUNTER_TYPE_APP_LENS) == 0) && m_ucMotorLensAPPEnable && palMotor_Lens_Shift_Status_Get() == FALSE) //HICC2_Casper_0030 //HICC2_Casper_0033 //HICC2_Casper_0041
    {
...
// ==============================================================================
// FUNCTION NAME: palInputProc_CLIHANDLER
// DESCRIPTION:
//
//
// Params:
...
BOOL palInputProc_DDP_PatternCheck(void) //HICC2_Casper_0019 //HICC2_Casper_0020
{
...
BOOL palInputProc_ADV_WarpBackCheck(void)		//G100_Doulas_0027
{
    ...
    //if(utilWarp_GetWarpCtrl() == WARP_CTRL__ADVANCED)	//G100_Doulas_0089 remove
	{
		...
		{
            //halAdvWarpShowOsdPattern(PAT_TYPE__BLACKLEVEL_SEL_POINT);
            //halAdvWarpShowOsdPattern(PAT_TYPE__OFF);  //A65_OPTOMA_Doulas_0023
			bCheck = TRUE;
		}
		...
		else if(utilWarp_GetOsdPatternType() == PAT_TYPE__BLACKLEVEL_PREVIEW ||
                utilWarp_GetOsdPatternType() == PAT_TYPE__BLACKLEVEL_BOUNDARY)	//A65_OPTOMA_Doulas_0020   //H2PF_Simon_0155
		{
			bCheck = FALSE;  //H2PF_Simon_0111   //因需要判斷開啟的 menu , 由 iScalerH2CT 處理
		}
#endif
...
BOOL palInputProc_ADV_WarpDirectionCheck(UINT8 ucVal)		//G100_Doulas_0027
{
	...
	LOG_MSG(db_APP_INPUTKEY, "(func:%s, line:%d) %d\r\n", __func__, __LINE__, ucVal);

    if(utilWarp_GetWarpCtrl() == WARP_CTRL__ADVANCED)
	{
		if(utilWarp_GetOsdPatternType() == PAT_TYPE__WARP_SEL_CTRL_POINT)
		{
			palGeo_AdvWarp_SelectControlPoint((eDIR)ucVal);
			bCheck = TRUE;
		}
		else if(utilWarp_GetOsdPatternType() == PAT_TYPE__WARP_MOV_CTRL_POINT)
		{
			palGeo_AdvWarpGP_MoveControlPoint((eDIR)ucVal);
			bCheck = TRUE;
		}

		else if(utilWarp_GetOsdPatternType() == PAT_TYPE__SEL_BLEND_WIDTH)
		{
			palGeo_AdvBlend_SelectOsdBlendWidth((eDIR)ucVal);
			bCheck = TRUE;
		}

		else if(utilWarp_GetOsdPatternType() == PAT_TYPE__ADJ_BLEND_WIDTH)
		{
			palGeo_AdvBlend_AdjustOsdBlendWidth(ucVal);
		...
		{
			palGeo_AdvBlackLevel_SelectControlPoint((eADV_BKLV_DIR)ucVal);
			bCheck = TRUE;
		}
		else if(utilWarp_GetOsdPatternType() == PAT_TYPE__BLACKLEVEL_MOV_POINT)	//A65_OPTOMA_Doulas_0020
		{
			palGeo_AdvBlackLevel_MoveControlPoint((eADV_BKLV_DIR)ucVal);
			bCheck = TRUE;
		}
		else if(utilWarp_GetOsdPatternType() == PAT_TYPE__BLACKLEVEL_ADD_POINT)	//A65_OPTOMA_Doulas_0020
		{
    ...
    switch (m_ucOsdUnlock_Procedure_Status)
    {
        ...
        case OSD_UNLOCK_PROCEDURE_STEP4:
            if (dwKey == keLEFT) {
				...
				palDataMgr_UI_EventSend(edcUI_EVENT_OSD_LOCK, 2, NULL);// appGui_Open_OSD_Lock_Remind_Msg_Menu();
            }
            else {
                palInputProc_OSD_Unlock_Step_Set(OSD_UNLOCK_PROCEDURE_STEP1);
            }
...
eCMD_KEY_DATACODE_EXECUTE_STATUS palInputProc_OSD_Lock_Check_Key( eKEY_INPUT_TYPE eKeyType, UINT32 dwKey)
{
    ...
    if ( ( eRet == eEXEC_CODE_PASS ) && ( ucOSD_Lock_Status == eOSD_LOCKED ) )       // OSD is locking
    {
        ...
        {
			palDataMgr_UI_EventSend(edcUI_EVENT_OSD_LOCK, 1, NULL);// appGui_Open_OSD_Lock_Icon_Msg_Menu();
        }
    }
...
{
    UINT8 ucKeypadLock = 0;
    palDataMgr_Data_Access(edcKEYPAD_LOCK, edaREAD, &ucKeypadLock);
    if((ucKeypadLock > 0) && (psInputKey->eKeyType == eKEYINPUT_KEYPAD))
    {
        return(TRUE);
    }
    return(FALSE);
...
BOOL palInputProc_Keypad_UnLock_Input_Check(sKEY_DATA *psInputKey)
{
    ...
    switch(psInputKey->wKeyCode)
    {
        ...
        case keHOLD_ENTER:
            if(palInputProc_KeyTimerHander(psInputKey) == TRUE)
            {
                if(ucUnlockCnt < KEYPAD_UNLOCK_CNT)
                {
                    ucUnlockCnt ++;
                }
            }
        ...
        case keRELEASE_ENTER:
            ...
            {
                UINT8 ucKeypadLock = 0;
                palDataMgr_Data_Access(edcKEYPAD_LOCK, edaWRITE_THROUGH_WITH_ACTION, &ucKeypadLock);
                bRet = TRUE;
            }
            ucUnlockCnt = 0;
            break;
...
#endif
#if defined(QUICK_KEY_ENABLE)
BOOL palInputProc_Quick_Key_Lock_Input_Check(sKEY_DATA *psInputKey)// HICC2_Bruce_0011
{
	UINT8 ucQuickKey = 0, ucQuickKeyLock = 0;
	palDataMgr_Data_Access(edcQUICK_KEY, edaREAD, (void*)&ucQuickKey);
	palDataMgr_Data_Access(edcQUICK_KEY_LOCK, edaREAD, (void*)&ucQuickKeyLock);
	if((ucQuickKey > 0) && (ucQuickKeyLock > 0))
	{
		if((psInputKey->wKeyCode == keSTANDBY) || (psInputKey->eKeyType == eKEYINPUT_KEYPAD && psInputKey->wKeyCode == kePOWERON))
		{
		}
		else if(ucQuickKey == (psInputKey->wKeyCode - ke0))
		{
		}
		else
		{
...
// ==============================================================================
// FUNCTION NAME: palInputProc_RGB_LevelPoll
// DESCRIPTION:
//
//
// Params:
...
// modification history
// --------------------
// 11/16/2022, Doulas Create
// --------------------
// ==============================================================================
INT16 palInputProc_RGB_LevelPoll(UINT16 uiTick)	//A70LK_Doulas_0012
{
	//uint16 RGB_LEVEL_Max[3],RGB_LEVEL_Min[3];	//A70LK_Doulas_0016

	if((palCoreVar_GetLightSourceOnVar() == TRUE) && //light source on
		(palSystem_PowerStateGet() == ePOWER_STATE_ACTIVE))
	{
	//	halScaler_FpgaDataRGBLevelGet(RGB_LEVEL_Max,RGB_LEVEL_Min);		//A70LK_Doulas_0016 remove
	//	LOG_MSG(db_APP_POLL, "RGB min(%d,%d,%d),(%d,%d,%d)\r\n",RGB_LEVEL_Min[0],
	//				                                            	RGB_LEVEL_Min[1],
	//				                                            	RGB_LEVEL_Min[2],
	//				                                            	RGB_LEVEL_Max[0],
	//				                                            	RGB_LEVEL_Max[1],
	//				                                            	RGB_LEVEL_Max[2]); //A70LK_Doulas_0016
	}
	return INPUT_RGB_LEVEL_POLLING_PERIOD / POLL_PERIOD ;
}
...
BOOL palInputProc_AdvWarpPatternOn_KeyCheck(UINT32 eKey)
{
    ...
    switch(eKey)
    {
        case keUP:
        case keHOLD_UP:
#if (ADVANCED_WARP_OSD == 1)
            if(palInputProc_ADV_WarpDirectionCheck(DIR__UP) == TRUE)
            {
                bValue = TRUE;
            }
#endif
        ...
        case keHOLD_DOWN:
#if (ADVANCED_WARP_OSD == 1)
            if(palInputProc_ADV_WarpDirectionCheck(DIR__DOWN) == TRUE)
            {
                bValue = TRUE;
            }
#endif
        ...
        case keHOLD_LEFT:
#if (ADVANCED_WARP_OSD == 1)
            if(palInputProc_ADV_WarpDirectionCheck(DIR__LEFT) == TRUE)
            {
                bValue = TRUE;
            }
#endif
        ...
        case keHOLD_RIGHT:
#if (ADVANCED_WARP_OSD == 1)
            if(palInputProc_ADV_WarpDirectionCheck(DIR__RIGHT) == TRUE)
            {
                bValue = TRUE;
            }
#endif
        ...
        case keENTER:
#if (ADVANCED_WARP_OSD == 1)
            if(palInputProc_ADV_WarpEnterCheck() == TRUE)
            {
                bValue = TRUE;
            }
#endif
        ...
        case keMENU:
            if(palInputProc_ADV_WarpBackCheck() == TRUE)
            {
                bValue = TRUE;
            }
            break;
        ...
        case keLENSVDOWN:
    		...
    		{
    			UINT8 ucData = FALSE;
    			palDataMgr_Data_Access(edcAW_WARP_PATTERN_SHOW, edaWRITE_THROUGH_WITH_ACTION, &ucData);
                bValue = TRUE;
    		}
            break;
    }
...
{
	m_cFOTA_Status = ucStatus;
}

UINT8 palInput_FOTA_StatusGet(void)
{
	return m_cFOTA_Status;
}

void palInput_Lens_Type_Set(UINT8 ucData) //HICC2_Julie_0071//H30K_Julie_0005
{
    m_ucLensType = ucData;
}

UINT8 palInput_Lens_Type_Get(void) //HICC2_Julie_0071//H30K_Julie_0005
{
    return m_ucLensType;
}

void palInput_Fuji_Lens_Detect_Set(UINT8 ucData) //HICC2_Julie_0071//H30K_Julie_0005
{
    m_ucFujiLensDetect = ucData;
}

UINT8 palInput_Fuji_Lens_Detect_Get(void) //HICC2_Julie_0071//H30K_Julie_0005
{
    //1: screw not lock.
    //0: screw locked.
    return m_ucFujiLensDetect;
}
