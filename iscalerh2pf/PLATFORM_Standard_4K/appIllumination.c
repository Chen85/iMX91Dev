...
static BOOL bBurnIn_PowerOff = FALSE;
#endif

#ifdef OE_JIG //HICC2_Doulas_0096//A70LV_Larry_0322
#define OE_JIG_FAN_LOCK_THRESHOLD_RPM                       500                 //HICC2_Tim_0001, add, ***
#define OE_JIG_FAN_LOCK_THRESHOLD_COUNT                     5
#define OE_JIG_DMD_TEMP_THRESHOLD                           7500
#define OE_JIG_DMD_TEMP_THRESHOLD_COUNT                     5                   //HICC2_Tim_0001, add, &&&

static UINT8 m_ucFan1ErrorCount  = 0;
static UINT8 m_ucFan2ErrorCount  = 0;
static UINT8 m_ucFan3ErrorCount  = 0;
static UINT8 m_ucFan4ErrorCount  = 0;   //HICC2_Tim_0001, add, ***
static UINT8 m_ucFan5ErrorCount  = 0;
static UINT8 m_ucFan6ErrorCount  = 0;   //HICC2_Tim_0001, add, &&&
static UINT8 m_ucFan7ErrorCount  = 0;
static UINT8 m_ucFan8ErrorCount  = 0;
static UINT8 m_ucFan10ErrorCount = 0;
static UINT8 m_ucFan11ErrorCount = 0;
static UINT8 m_ucFan14ErrorCount = 0;
static UINT8 m_ucFan15ErrorCount = 0;
static UINT8 m_ucFan12ErrorCount = 0;
static UINT8 m_ucFan17ErrorCount = 0;

static UINT8 m_ucPumpErrorCount = 0;
static UINT8 m_ucTecDMDErrorCount = 0;
static UINT8 m_ucTecRLDErrorCount = 0;
static UINT8 m_ucDMDTempErrorCount = 0;
...
static UINT32  m_dwOPDPeriodCnt = 0;  //HICC2_Steven_0030

//eDDP_PICTURE_SETTINGS_ID GEC_MapToDDP_PicMode(eIFC_WAP_PresetMode ePicMode);

#define SMOOTH_ON_DELAY (3)
#define SMOOTH_ON_READY (2)
#define SMOOTH_ON_DONE  (0)

#define A70_LENS_STYLE      //HICC2_Doulas_0103
...
eEXEC_CODE palIllumination_Init(void)
{
	...
	m_sPalIllumInfo.ucSmoothOn = SMOOTH_ON_DELAY;
	m_sPalIllumInfo.ucIRSmoothOn = SMOOTH_ON_DELAY; //T100IR_Casper_0024
    m_sPalIllumInfo.cLensPlug = 99; //A70LV_Larry_0340
    m_sPalIllumInfo.ucRentalModeEnable = 0;
    m_sPalIllumInfo.ucLensDetection = 1;
//    psGSENSOR_MODE.cMode = 15;	//A70LK_Doulas_0005
//    psGSENSOR_MODE.cPortrait= 1;	//A70LK_Doulas_0005
    m_sPalIllumInfo.cWorkCounterCheck = PHASE_LOCK_COUNT_DOWN;
    m_sPalIllumInfo.cPhaseLockMaxCounterCheck = 0;

//read burnin finformation for lamp time count with burnin function
...
eEXEC_CODE palIllumination_PowerNormal(void)
{
    ...
    palSystem_VersionCheck(FALSE); //A70LV_Larry_0112
    ...
    if(palSystem_NoFanControlFlagGet() == TRUE) //A70LV_Larry_0131
    {
        //halFanCtrl_NoFanControlSet();
        m_sPalIllumInfo.ucLensDetection = 0;
	//Cassper_ProAV
        //halLDCtrl_DisableShutdown();
...
#ifdef OE_JIG
    m_ucFan1ErrorCount  = 0; //HICC2_Doulas_0096
    m_ucFan2ErrorCount  = 0;
    m_ucFan3ErrorCount  = 0;
    m_ucFan4ErrorCount  = 0;            //HICC2_Tim_0001, add, ***
    m_ucFan5ErrorCount  = 0;
    m_ucFan6ErrorCount  = 0;            //HICC2_Tim_0001, add, &&&
    ...
    m_ucDMDTempErrorCount = 0; //HICC2_Doulas_0096
    m_ucOE_JIG_Error = 0;

    m_sPalIllumInfo.ucLensDetection = 0;
    LOG_MSG(db_ALWAYS, "Enter no fan control\r\n");

    //halFanCtrl_SkuReload_Set(eMODEL_TYPE_4K22HS); //HICC2_Doulas_0096 remove
    ...
    //ucData = 1;
    //palDataMgr_Data_Access(edcHIGH_ALTITUDE, edaWRITE_RAM_ONLY_WITH_ACTION, (void*)&ucData); //HICC2_Doulas_0096
#endif
...
//    psGSENSOR_MODE.cPortrait= 1;	//A70LK_Doulas_0005
    ...
    m_sPalIllumInfo.ucDelaySmoothOn = 10;
	...
	m_sPalIllumInfo.ucSmoothOn = SMOOTH_ON_DELAY;
	m_sPalIllumInfo.ucIRSmoothOn = SMOOTH_ON_DELAY;
	...
	halMCUCtrl_Standby_Error_Get(&ucFlag); //HICC2_Steven_0029
...
eEXEC_CODE palIllumination_PowerStandby(void)
{
    ...
    */
	...
	halFormatter_Standby(); //T100_Casper_0048
...
eEXEC_CODE palIllumination_RLD_ValueCheck(UINT32 dwY_Hi, UINT32 dwY_Lo, UINT32 dwX_In, UINT32 dwX_Hi, UINT32 dwX_Lo) 	//G100_Doulas_0023
{
    if(dwX_Hi == dwX_Lo)
    {   // avoid overflow
...
void palIllumination_LghtSourceMode_ConstantIntensity(void) //T100_Larry_0022 //A70G2_Owen_0003
{
	...
	{
		if(palIllumination_RLD_ValueCheck(uiFull_Dynamic_RLD_PWM[ucCount],
...
void palIllumination_ReduceLighthandler(void) //T100_Larry_0022 //A70G2_Owen_0003
{
   ...
   // UINT16 uiCurrent_PWM[eLD_SEQ_NUMBER] = {0};
    ...
    ulTargetLight = (UINT32)((uiTarget_BLD_LightSensor[eLD_SEQ_G]*m_sPalIllumInfo.uiABPTargetBLDLight[eLD_SEQ_Y])/m_sPalIllumInfo.uiABPTargetBLDLight[eLD_SEQ_G]);  //H30K_Tim_0007, add
    m_sPalIllumInfo.uiABPTargetBLDLight[eLD_SEQ_Y] = (UINT16)ulTargetLight;     //H30K_Tim_0007, add
...
void palIllumination_AbpLightSensor(sLIGHT_SENSOR *psLightSenosr) //A70LV_Larry_0142 //A70G2_Owen_0003
{
    ...
    LOG_MSG(db_APP_ILLUMINATION, "Light Sensor [Y/R/B/G/W]%d,%d,%d,%d,%d\r\n",
     psLightSenosr->uiY,psLightSenosr->uiR,psLightSenosr->uiB,psLightSenosr->uiG,psLightSenosr->uiW);
		...
		case eLIGHT_SENSOR_CAL_SAMPLE_TIME_RLD_VALUE:	//A65_OPTOMA_Doulas_0106 //A70G2_Owen_0002
            {
                ...
                palDataMgr_Data_Access(edcLIGHTSENSOR_TARGET_FACTORY, edaREAD, (void*)auiRLD);
    			halLDCtrl_T0LightSensorValue_Get(eLDBANK_A70LV, (UINT8 *)auiADC);

				LOG_MSG(db_APP_ILLUMINATION, "auiRLD (%d) auiADC(%d)\r\n",auiRLD[eLD_SEQ_R], auiADC[eLD_SEQ_R]);
    			halLDCtrl_LS_RLD_Enable_Set(TRUE);
        ...
        case eLIGHT_SENSOR_INTESSITY_START:
            {
                ...
                if(ucError)
                {
                   ...
                   uOPDData.sSnapshotLog.wLightSenosr[0] = sLightSenosr.uiY;
                   uOPDData.sSnapshotLog.wLightSenosr[1] = sLightSenosr.uiR;
                   uOPDData.sSnapshotLog.wLightSenosr[2] = sLightSenosr.uiB;
                   uOPDData.sSnapshotLog.wLightSenosr[3] = sLightSenosr.uiG;
                   utilOPD_EventSet(eOPD_LIGHT_SENSOR_ERROR_LOG, &uOPDData);
                }
                else
                {
...
//                uiData[eLD_SEQ_G] = sLightSenosr.uiG; //G100_Owen_0092 remove
                ...
                {
                   if((ABC_LIGHT_SENSOR_BLD_MINIMUM > uiData[ucCount]) || (ABC_LIGHT_SENSOR_MAXIMUM < uiData[ucCount]))
                   {
                       ucError |= 1;
                   }
                }
...
void palIllumination_CCT_Set(void)
{
    ...
    utilDataMgr_CCT_StorageGet(&uCCTTable); //HICC2_Casper_0014
...
{
    palDataMgr_TecGatingInfo_Get(&sTEC_GATING);
}
...
void palIllumination_TEC_Gating_InfoSet( UINT16 cCycleData)
{
    ...
    palDataMgr_TecGatingInfo_Set(&sTEC_GATING);
...
static void palIlluminationLightSourceCount(void)
{
	...
	palDataMgr_Data_Access(edcLD_HOURS_FACTORY, edaREAD, &ulVal);// HICC2_Bruce_0012
	ulVal += 1;
	palDataMgr_Data_Access(edcLD_HOURS_FACTORY, edaWRITE_THROUGH_NO_ACTION, &ulVal);
	...
	palLANProcSendToLAN(edcLIGHT_SOURCE_HOURS_INFO);	//A70LK_Doulas_0014
	palLANProcSendToLAN(edcLD_HOURS_INFO);				//A70LK_Doulas_0014
	palLANProcSendToLAN(edcSERVICE_LIGHT_SOURCE_HOUR);	//A70LK_Doulas_0014
    ...
    else if(ucPowerMode == eCM_POWER_MODE_LOW_BRIGHTNESS_MODE)
    {
    ...
    else if(ucPowerMode == eCM_POWER_MODE_ECO1)
    {
	...
	else if(ucPowerMode == eCM_POWER_MODE_CUSTOM_MODE)
	{
		palDataMgr_Data_Access(edcLIGHT_SOURCE_HOURS_CUSTOM, edaREAD, &ulVal);
		ulVal = ulVal + 1;
		palDataMgr_Data_Access(edcLIGHT_SOURCE_HOURS_CUSTOM, edaWRITE_THROUGH_NO_ACTION, &ulVal);
...
INT16 palIllumination_Poll(UINT16 uiTick)
{
    ...
    DDP_SYSTEM_STATUS sDDPSystemStatus;                         //A70LV_Doulas_0212
    eRESULT eResult = rcINVALID;                                //A70LV_Doulas_0212
    UINT8 ucValue = 0;
    UINT8 ucRear = 0;
	UINT8 bPowerNum = 0; //T100IR_Casper_0024
	UINT8 ucCeilingMount = 99;	//A70LK_Doulas_0003

	UINT16 wOPD_Period = palDataMgr_OPD_Period_Get();  //HICC2_Steven_0030

#ifdef A70_LENS_STYLE
    ...
    if(Board_SingleBoard_Get() == FALSE)
    {
        if(bDDP_AsicReady)
        {
            ...
            {
                if(m_sPalIllumInfo.ucDelaySmoothOn)
                {
                    m_sPalIllumInfo.ucDelaySmoothOn--;
                    return ILLUMINATION_PERIOD/POLL_PERIOD;
                }
                ...
                if(m_sPalIllumInfo.ucSmoothOn == SMOOTH_ON_DELAY) //A70LV_Larry_0255
                {
                    ...
                    LOG_MSG(db_APP_ILLUMINATION,"PhaseLock1 %d(%d,%d)(%d,%d)(%d)\r\n",
                                                         m_sPalIllumInfo.cPhaseLockMaxCounterCheck
                    ...
                    {
                        m_sPalIllumInfo.cWorkCounterCheck--;
                    }
                    else
                    {
                        m_sPalIllumInfo.cWorkCounterCheck = PHASE_LOCK_COUNT_DOWN;
                    ...
                    //if(m_sPalIllumInfo.cPhaseLockMaxCounterCheck % 10 == 8)
                    //{
                    //    halMCU_DDP_PowerSet(1);
                    //}
                    m_sPalIllumInfo.cPhaseLockMaxCounterCheck++;

                    if((m_sPalIllumInfo.cWorkCounterCheck <= 0) || (m_sPalIllumInfo.cPhaseLockMaxCounterCheck > PHASE_LOCK_MAX_TIMER))     //A70LV_Doulas_0212 DDP phase lock check
                    {
                        ...
                        while(eEXEC_CODE_PASS != palDataPath_GetPathReady()) //等DataPath ready在點燈，套twist blending會比較久
                        {
                            MS_SLEEP(100);
                        }

                        //appDataMgr_ColorConfig_Parameter_Set();
                        palDataMgr_Format_PictureMode_Set(); //T100_Casper_0087
#ifdef OE_JIG
                    #if defined(PLATFORM_R70K)            //HICC2_Tim_0005, add, ***
                        palIllumination_OE_JIG_Light_Source_Init();
                    #else //defined(PLATFORM_R70K)        //HICC2_Tim_0005, add, &&&
                        palDataMgr_LightSource_Set(eCM_POWER_MODE_20, 100); //HICC2_Doulas_0096//13 is eCM_POWER_MODE_20
                    #endif //defined(PLATFORM_R70K)       //HICC2_Tim_0005, add
#endif /* OE_JIG */
#if OPEN_WAP
                        ...
                        if(rcSUCCESS == halMCUCtrl_AC_InputGet(&ucV110Det))
                        {
                ...
                else
                {
                    ...
                    #ifdef OE_JIG       //HICC2_Tim_0008, add, ***
                      #if defined(PLATFORM_R70K)
                        halFormatter_Projection_Mode_Set(eCMD_Formatter_External);
                      #endif //defined(PLATFORM_R70K)
                    #endif //OE_JIG     //HICC2_Tim_0008, add, &&&
                    ...
                    else if(m_sPalIllumInfo.ucSmoothOn == SMOOTH_ON_READY) //A70LV_Larry_0340
                    {
						...
						halFormatter_ALC_RGB_LevelGet(RGBLevel);
						palFormatterMgr_Projection_Mode_Get(&ucMasterProjectMode);
        				palFormatterMgr_SlaveProjection_Mode_Get(&ucSlaveProjectMode);
						LOG_MSG(db_APP_SYSTEM, "@@@@ RGBLevel %d,%d,%d,%d,%d,%d\r\n",RGBLevel[0],RGBLevel[1],RGBLevel[2],RGBLevel[3],RGBLevel[4],RGBLevel[5]);
						LOG_MSG(db_APP_SYSTEM, "@@@@ ProjectMode,%d,%d\r\n",ucMasterProjectMode,ucSlaveProjectMode);

						halFormatter_XPR_ErrorStatus_Get(&ucXPRErrorStatus);

                        LOG_MSG(db_APP_SYSTEM, "(%s, %d) XPRErrorStatus = %d\n", __FUNCTION__, __LINE__, ucXPRErrorStatus);

                        if((ucXPRErrorStatus) ||
							(RGBLevel[1] < 64 && RGBLevel[3] < 64 && RGBLevel[5] < 64 ) ||
							(ucMasterProjectMode != eCMD_Formatter_External || ucSlaveProjectMode != eCMD_Formatter_External))
                        {
                            halFormatter_XPR_ErrorStatus_Set();
                        }

						if(palMotor_LensDetection_State_Get() == eMLC_LSM_DETECTION_UNPLUGED)
							palLedProc_LED_Behavior_Set(eLED_STATUS_LENS_UNPLUGGED);
						else if(palMotor_LensDetection_State_Get() == eMLC_LSM_DETECTION_MONITOR)
                        ...
                        else
                        {
...
#ifdef A70_LENS_STYLE
    						if(m_sPalIllumInfo.cLensPlug != cLensPlug)	//G100_Doulas_0076 Modify 等待palDataPath_StartDisplay 跑完加上2秒後點燈
#else
...
#endif
                            {
...
#ifdef A70_LENS_STYLE
                                m_sPalIllumInfo.cLensPlug = cLensPlug;
                                if(m_sPalIllumInfo.cLensPlug) //LD off
                                {
                                    m_sPalIllumInfo.ucSmoothOn--;
                                    palLedProc_Shutter_LED_Set(eLED_SHUTTER_ON);
    								//halLDCtrl_LD_SourceEnable_Set(eLDBANK_A70LV, 0);	//A70LK_Larry_0120 mask//G100_Clare_0005
                                ...
                                { //A35G2_BRC_Casper_0116
                                    ...
                                    if((ucID == eLENS_ID_B20U100) || (ucID == eLENS_ID_NVRAM_B20U100)) //HICC2_Julie_0071//H30K_Julie_0005
                                    {
	...
	if(wOPD_Period != 0 && (m_sPalIllumInfo.ulCurrentRunTime >= 2))   //HICC2_Steven_0030
	{
		//LOG_MSG(db_APP_ILLUMINATION,"palDataMgr_OPDSnapshot (%d, %d)\r\n", m_dwOPDPeriodCnt , m_sPalIllumInfo.ulCurrentRunTime);
		if( (palSystem_PowerStateGet() == ePOWER_STATE_ACTIVE))
		{
			if(m_dwOPDPeriodCnt >= wOPD_Period)
			{
				LOG_MSG(db_APP_ILLUMINATION,"OPD Engine log \r\n");
				m_dwOPDPeriodCnt = 0;
    ...
    {
		UINT32 ulVal = 0;// HICC2_Bruce_0012
        palDataMgr_Data_Access(edcTOTAL_PROJECTOR_HOURS_FACTORY, edaREAD, &ulVal);
        ulVal += 1;
        palDataMgr_Data_Access(edcTOTAL_PROJECTOR_HOURS_FACTORY, edaWRITE_THROUGH_WITH_ACTION, &ulVal);
        ...
        if((palSystem_PowerStateGet() == ePOWER_STATE_ACTIVE) && (m_sPalIllumInfo.ulCurrentRunTime%10 == 0) && (wOPD_Period== 0))
        {
            palDataMgr_OPDSnapshot(eOPD_ENGINE_LOG);
        }
	...
	palDataMgr_SecurityTimer_Poll();// HICC2_Bruce_0016
#ifdef A70_LENS_STYLE
    ...
    {
        ulLiteTime += 1; //T100_Casper_0032
        #if 0
        if((m_sPalIllumInfo.uiLampUpdateCount %30) == 1)//A70LV_Doulas_0152 test
        {
                static TickType_t tTime1 = 0;
                static TickType_t tTime2 = 0;
                if(tTime1 > tTime2)
                {
                    tTime2 = xTaskGetTickCount();
                    LOG_MSG(db_ALWAYS, "Total1<%ld>(0x%x,0x%x)\r\n" , (tTime2 -tTime1) / portTICK_RATE_MS ,tTime1,tTime2);

                }
                else
                {
                    tTime1 = xTaskGetTickCount();
                    LOG_MSG(db_ALWAYS, "Total2<%ld>(0x%x,0x%x)\r\n" , (tTime1 -tTime2) / portTICK_RATE_MS,tTime1,tTime2 );
                }
        }
        #endif
        if( ++m_sPalIllumInfo.uiLampUpdateCount >= LAMP_PERIOD_UPDATE )  //One minute //A70LV_Doulas_0061 modify
        {
            ...
            if(m_sPalIllumInfo.sBurnIn_Info.ucBurnInEnable == 2)// HICC2_Bruce_0009 // 2: Always Lamp On
            {
                //LOG_MSG(db_ALWAYS, "%s(eBURNIN_TYPE_ALWAYS_ON)\r\n", __FUNCTION__);
            }
            else if(m_sPalIllumInfo.sBurnIn_Info.ucBurnInEnable)
            {
                #ifdef PLATFORM_H30_4K
                if((m_sPalIllumInfo.ulStartUpLampPeriod >= m_sPalIllumInfo.sBurnIn_Info.ucLampOn) && (bBurnIn_PowerOff))
				#else
...
#ifdef PLATFORM_H30_4K// HICC2_Bruce_0009
    if(bIsLightSourceOn && (0 == m_sPalIllumInfo.uiLampUpdateCount%5) && (m_sPalIllumInfo.sBurnIn_Info.ucBurnInEnable == 2))
    {
...
#if defined(PLATFORM_H30_4K) //HICC2_AC_0001
    //OE_JIG no fan control
#else

#ifdef OE_JIG //A70LV_Larry_0322
    if((bIsLightSourceOn) && (m_sPalIllumInfo.uiLampUpdateCount%10 == 0)) //HICC2_Doulas_0096 && (palSystem_SystemCheckGet() == FALSE))
    {

    #if defined(PLATFORM_R70K)          //HICC2_Tim_0001, add, ***

        palIllumination_OE_JIG_FAN_Monitor();

    #else //defined(PLATFORM_R70K)      //HICC2_Tim_0001, add, &&&

        UINT16 uiFan1  = palDataMgr_Access_Get_FAN_RPM(0);
        UINT16 uiFan2  = palDataMgr_Access_Get_FAN_RPM(1);
        UINT16 uiFan3  = palDataMgr_Access_Get_FAN_RPM(2);
        UINT16 uiFan7  = palDataMgr_Access_Get_FAN_RPM(6);
        UINT16 uiFan8  = palDataMgr_Access_Get_FAN_RPM(7);
        UINT16 uiFan10 = palDataMgr_Access_Get_FAN_RPM(9);
        UINT16 uiFan11 = palDataMgr_Access_Get_FAN_RPM(10);
        UINT16 uiFan12 = palDataMgr_Access_Get_FAN_RPM(11);
        UINT16 uiFan14 = palDataMgr_Access_Get_FAN_RPM(13);
        ...
        if( (uiFan1  < 500) || (uiFan2 < 500)  || (uiFan3  < 500) || (uiFan7 < 500) ||
            ...
            (uiFan11 < 500) || (uiFan12 < 500) || (uiFan15 < 500) || (uiFan17 < 500)) //H60_Doulas_0038//H60_Doulas_0024 Modify
        {
            if(uiFan1 < 500)
            {
                m_ucFan1ErrorCount++;
            ...
            if( m_ucFan1ErrorCount  >= 5 || m_ucFan2ErrorCount  >= 5 || m_ucFan3ErrorCount  >= 5 || m_ucFan7ErrorCount >= 5 ||
                ...
                m_ucFan11ErrorCount >= 5 || m_ucFan12ErrorCount >= 5 || m_ucFan15ErrorCount >= 5 || m_ucFan17ErrorCount >= 5)
            {
                m_ucOE_JIG_Error = 1;

                LOG_MSG(db_ASSERT, "Fan Lock = (%d %d %d %d)(%d %d %d %d)(%d %d %d)\n", uiFan1, uiFan2, uiFan3, uiFan7, uiFan8, uiFan10, uiFan11, uiFan12, uiFan14, uiFan15, uiFan17); //H60_Doulas_0038
            }
        }
        else
        {
        ...
        //check DMD Temp
        if(iDMDTemp > 7500) //H60_Doulas_0024
        {
        ...
        if(m_ucOE_JIG_Error)
        {
            LOG_MSG(db_ASSERT, "OE JIG Shut Down\r\n");       //HICC2_Tim_0001, add
            palSystem_PowerDown();
        }

#endif //defined(PLATFORM_R70K)     //HICC2_Tim_0001, add
    ...
    {
		UINT8  ucXPR_ClosedLoopEnable0 = 0;
        UINT8  ucXPR_ClosedLoopEnable1 = 0;
	    UINT32 ucXPR_Gain0 = 0;
	    UINT32 ucXPR_Gain1 = 0;
	    UINT32 ucXPR_SegmentLength0 = 0;
	    UINT32 ucXPR_SegmentLength1 = 0;
	    UINT32 ucXPR_SN = 0;
	    UINT16 uiDataCode = 0;
	    UINT32 ucXPR_ADC0 = 0;
	    UINT32 ucXPR_ADC1 = 0;

        palDataMgr_Data_Access(edcCLOSE_LOOP_ENABLE_AXIS0, edaREAD, &ucXPR_ClosedLoopEnable0);
        palDataMgr_Data_Access(edcCLOSE_LOOP_ENABLE_AXIS1, edaREAD, &ucXPR_ClosedLoopEnable1);

        if(ucXPR_ClosedLoopEnable0)
        {
            palFormatterMgr_XPR_Gain_of_Axis0_Get(&ucXPR_Gain0);
            palDataMgr_Data_Access(edcCLOSE_LOOP_GAIN_AXIS0, edaWRITE_RAM_ONLY_NO_ACTION, &ucXPR_Gain0);

            palFormatterMgr_XPR_Segment_Length_of_Axis0_Get(&ucXPR_SegmentLength0);
            palDataMgr_Data_Access(edcCLOSE_LOOP_SEGMENT_LENGTH_AXIS0, edaWRITE_RAM_ONLY_NO_ACTION, &ucXPR_SegmentLength0);
        }

        if(ucXPR_ClosedLoopEnable1)
        {
            palFormatterMgr_XPR_Gain_of_Axis1_Get(&ucXPR_Gain1);
            palDataMgr_Data_Access(edcCLOSE_LOOP_GAIN_AXIS1, edaWRITE_RAM_ONLY_NO_ACTION, &ucXPR_Gain1);

            palFormatterMgr_XPR_Segment_Length_of_Axis1_Get(&ucXPR_SegmentLength1);
            palDataMgr_Data_Access(edcCLOSE_LOOP_SEGMENT_LENGTH_AXIS1, edaWRITE_RAM_ONLY_NO_ACTION, &ucXPR_SegmentLength1);
        }

        if( m_sPalIllumInfo.ulCurrentRunTime%5 ==0) //every 5 min //HICC2_Steven_0051 start
        {
        	 UINT32 ucXPR_ErrCode0 = 0, ucXPR_ErrCode1 = 0;

        	 if(ucXPR_ClosedLoopEnable0)
        	 {
        		 palFormatterMgr_XPR_CloseLoop_ErrorCode0(&ucXPR_ErrCode0);
        		 palDataMgr_Data_Access(edcXPR_CLOSELOOP_ERR_CODE0, edaWRITE_RAM_ONLY_NO_ACTION, &ucXPR_ErrCode0);
        	 }

        	 if(ucXPR_ClosedLoopEnable1)
        	 {
        		 palFormatterMgr_XPR_CloseLoop_ErrorCode1(&ucXPR_ErrCode1);
        		 palDataMgr_Data_Access(edcXPR_CLOSELOOP_ERR_CODE1, edaWRITE_RAM_ONLY_NO_ACTION, &ucXPR_ErrCode1);
        	 }
        ...
        if(m_sPalIllumInfo.ucFanPollCount%10 == 0)
        {
            UINT16 uiDataCode = 0;
            UINT8 ucFanFilter  = 0;
            UINT8 ucFanNum = Syscfg_Value_Get_Typeint(eFan_INFO_Num_defined_in_MCU); //HICC2_Doulas_0091
		    UINT8 ucNum = 0; //HICC2_Doulas_0091

            m_sPalIllumInfo.ucFanPollCount = 0;
            for(ucNum = 0 ; ucNum < ucFanNum ; ucNum++) //HICC2_Doulas_0091
			{
				if(ucNum <= 15)
            ...
            for(uiDataCode = edcTHERMAL_SENSOR_1; uiDataCode <= edcTHERMAL_SENSOR_8; uiDataCode++) //HICC2_Doulas_0092//HICC2_Doulas_0091
            {
                palLANProcSendToLAN((eDATA_CODE)uiDataCode);
            }
...
eEXEC_CODE palIllumination_Init_LDHoursSet(UINT32 ulHours)   //A70LV_Doulas_0128 //A70LV_Larry_0268
{
    ...
    //{
        ...
        //palDataMgr_Access_LD_Hours(edaWRITE_THROUGH_NO_ACTION, &ulHours); //HICC2_Steven_0025
    //}
...
eEXEC_CODE palIllumination_Init_TotalProjectHoursSet(UINT32 ulHours)   //A70LV_Doulas_0128 //A70LV_Larry_0268
{
    ...
    //{
        ...
        //palDataMgr_Access_TotalProjectorHours(edaWRITE_THROUGH_NO_ACTION, &ulHours); //HICC2_Steven_0025
    //}
...
void palIllumination_InstantOff(void)
{
    ...
    //m_sPalIllumInfo.bIllumEnabled = FALSE;
    //appCoreVar_SetLightSourceOnVar(FALSE);
}
...
{
    UINT16 uiFan1  = palDataMgr_Access_Get_FAN_RPM(0);
    UINT16 uiFan4  = palDataMgr_Access_Get_FAN_RPM(3);
    UINT16 uiFan5  = palDataMgr_Access_Get_FAN_RPM(4);
    UINT16 uiFan6  = palDataMgr_Access_Get_FAN_RPM(5);
    UINT16 uiFan7  = palDataMgr_Access_Get_FAN_RPM(6);
    UINT16 uiFan8  = palDataMgr_Access_Get_FAN_RPM(7);      //HICC2_Tim_0002, add

    INT16  iDMDTemp = palDataMgr_Access_Get_Thermal_Sensor(1);

    UINT8 i = 0;

    LOG_MSG(db_APP_MONITOR, "R70K OE JIG, FAN RPM, (1,4,5) (6,7,8) = (%d,%d,%d) (%d,%d,%d)\r\n", uiFan1,uiFan4,uiFan5,uiFan6,uiFan7,uiFan8);    //HICC2_Tim_0002, mod
    LOG_MSG(db_APP_MONITOR, "R70K OE JIG, DMD Temp = %d\r\n", iDMDTemp);

    //check fan lock
    if( ( uiFan1 < OE_JIG_FAN_LOCK_THRESHOLD_RPM ) || ( uiFan4 < OE_JIG_FAN_LOCK_THRESHOLD_RPM )
     || ( uiFan5 < OE_JIG_FAN_LOCK_THRESHOLD_RPM ) || ( uiFan6 < OE_JIG_FAN_LOCK_THRESHOLD_RPM )
     || ( uiFan7 < OE_JIG_FAN_LOCK_THRESHOLD_RPM ) || ( uiFan8 < OE_JIG_FAN_LOCK_THRESHOLD_RPM )    //HICC2_Tim_0002, mod
    )
    {
        if(uiFan1 < OE_JIG_FAN_LOCK_THRESHOLD_RPM)
        {
            m_ucFan1ErrorCount++;

            LOG_MSG(db_APP_MONITOR, "R70K OE JIG, FAN1 Lock Count: %d\r\n", m_ucFan1ErrorCount);

            if(m_ucFan1ErrorCount >= OE_JIG_FAN_LOCK_THRESHOLD_COUNT)
            {
                utilDataMgr_WriteGecLog(FanLock01);
            }
        }
        ...
        if(uiFan4 < OE_JIG_FAN_LOCK_THRESHOLD_RPM)
        {
            m_ucFan4ErrorCount++;

            LOG_MSG(db_APP_MONITOR, "R70K OE JIG, FAN4 Lock Count: %d\r\n", m_ucFan4ErrorCount);

            if(m_ucFan4ErrorCount >= OE_JIG_FAN_LOCK_THRESHOLD_COUNT)
            {
                utilDataMgr_WriteGecLog(FanLock04);
            }
        }
        else
        {
            m_ucFan4ErrorCount = 0;
        }

        if(uiFan5 < OE_JIG_FAN_LOCK_THRESHOLD_RPM)
        {
            m_ucFan5ErrorCount++;

            LOG_MSG(db_APP_MONITOR, "R70K OE JIG, FAN5 Lock Count: %d\r\n", m_ucFan5ErrorCount);

            if(m_ucFan5ErrorCount >= OE_JIG_FAN_LOCK_THRESHOLD_COUNT)
            {
                utilDataMgr_WriteGecLog(FanLock05);
            }
        }
        else
        {
        ...
        if(uiFan6 < OE_JIG_FAN_LOCK_THRESHOLD_RPM)
        {
            m_ucFan6ErrorCount++;

            LOG_MSG(db_APP_MONITOR, "R70K OE JIG, FAN6 Lock Count: %d\r\n", m_ucFan6ErrorCount);

            if(m_ucFan6ErrorCount >= OE_JIG_FAN_LOCK_THRESHOLD_COUNT)
            {
                utilDataMgr_WriteGecLog(FanLock06);
            }
        }
        else
        {
            m_ucFan6ErrorCount = 0;
        }

        if(uiFan7 < OE_JIG_FAN_LOCK_THRESHOLD_RPM)
        {
            m_ucFan7ErrorCount++;

            LOG_MSG(db_APP_MONITOR, "R70K OE JIG, FAN7 Lock Count: %d\r\n", m_ucFan7ErrorCount);

            if(m_ucFan7ErrorCount >= OE_JIG_FAN_LOCK_THRESHOLD_COUNT)
            {
                utilDataMgr_WriteGecLog(FanLock07);         //HICC2_Tim_0003, mod
            }
        }
        else
        {
        ...
        //HICC2_Tim_0002, add, ***
        if(uiFan8 < OE_JIG_FAN_LOCK_THRESHOLD_RPM)
        {
            m_ucFan8ErrorCount++;

            LOG_MSG(db_APP_MONITOR, "R70K OE JIG, FAN8 Lock Count: %d\r\n", m_ucFan8ErrorCount);

            if(m_ucFan8ErrorCount >= OE_JIG_FAN_LOCK_THRESHOLD_COUNT)
            {
                utilDataMgr_WriteGecLog(FanLock08);
            }
        }
        ...
        if( ( m_ucFan1ErrorCount >= OE_JIG_FAN_LOCK_THRESHOLD_COUNT )
         || ( m_ucFan4ErrorCount >= OE_JIG_FAN_LOCK_THRESHOLD_COUNT )
         || ( m_ucFan5ErrorCount >= OE_JIG_FAN_LOCK_THRESHOLD_COUNT )
         || ( m_ucFan6ErrorCount >= OE_JIG_FAN_LOCK_THRESHOLD_COUNT )
         || ( m_ucFan7ErrorCount >= OE_JIG_FAN_LOCK_THRESHOLD_COUNT )
         || ( m_ucFan8ErrorCount >= OE_JIG_FAN_LOCK_THRESHOLD_COUNT )           //HICC2_Tim_0002, add
        )
        {
            m_ucOE_JIG_Error = 1;

            LOG_MSG(db_APP_MONITOR, "R70K OE JIG, FAN Lock Fail\r\n");
        }
    }
    else
    {
        ...
        m_ucFan6ErrorCount = 0;
        m_ucFan7ErrorCount = 0;
        m_ucFan8ErrorCount = 0;         //HICC2_Tim_0002, add
    }

    //check DMD Temp
    if(iDMDTemp > OE_JIG_DMD_TEMP_THRESHOLD)
    {
        m_ucDMDTempErrorCount++;

        LOG_MSG(db_APP_MONITOR, "R70K OE JIG, DMD Over Temp Count:%d\n", m_ucDMDTempErrorCount);

        if(m_ucDMDTempErrorCount >= OE_JIG_DMD_TEMP_THRESHOLD_COUNT)
        {
            m_ucOE_JIG_Error = 1;
            utilDataMgr_WriteGecLog(DMDOverTemp);
            LOG_MSG(db_APP_MONITOR, "R70K OE JIG, DMD Over Temp Fail\n");
        }
    }
    else
    {
    ...
    // shut down the system for OE JIG error
    if(m_ucOE_JIG_Error)
    {
        LOG_MSG(db_APP_MONITOR, "R70K OE JIG, Fail Shut Down\r\n");
        palSystem_PowerDown();
    }
}

//HICC2_Tim_0005, add, ***
void palIllumination_OE_JIG_Light_Source_Init(void)
{
    UINT8 ucOE_JIG_Value = eCM_POWER_MODE_CONSTANT_POWER;
    UINT16 uiOE_JIG_PWM_Init = 100;

    palDataMgr_Data_Access(edcPOWER_MODE, edaWRITE_THROUGH_WITH_ACTION, (void*)&ucOE_JIG_Value);
    ucOE_JIG_Value = 100;               //HICC2_Tim_0008, mod, 1
    palDataMgr_Data_Access(edcCONSTANT_POWER_NUMBER, edaWRITE_THROUGH_WITH_ACTION, (void*)&ucOE_JIG_Value);

    ucOE_JIG_Value = eCM_STANDBY_MODE_0_5W;
    palDataMgr_Data_Access(edcSTANDBY_MODE, edaWRITE_THROUGH_WITH_ACTION, (void*)&ucOE_JIG_Value); //work around the DC Power On without LD On issue.

    palDataMgr_Data_Access(edcFACTORY_FORMATTER_BLD_PWM_R, edaWRITE_RAM_ONLY_WITH_ACTION, &uiOE_JIG_PWM_Init);
    palDataMgr_Data_Access(edcFACTORY_FORMATTER_BLD_PWM_G, edaWRITE_RAM_ONLY_WITH_ACTION, &uiOE_JIG_PWM_Init);
    palDataMgr_Data_Access(edcFACTORY_FORMATTER_BLD_PWM_B, edaWRITE_RAM_ONLY_WITH_ACTION, &uiOE_JIG_PWM_Init);
    palDataMgr_Data_Access(edcFACTORY_FORMATTER_BLD_PWM_Y, edaWRITE_RAM_ONLY_WITH_ACTION, &uiOE_JIG_PWM_Init);

    //Set Bright Mode
    //ucOE_JIG_Value = eCM_PICTURE_SETTINGS_BRIGHT;
    //palDataMgr_Data_Access(edcPICTURE_SETTINGS, edaWRITE_RAM_ONLY_WITH_ACTION, &ucOE_JIG_Value);
}
//HICC2_Tim_0005, add, &&&
...
eDDP_PICTURE_SETTINGS_ID GEC_MapToDDP_PicMode(eIFC_WAP_PresetMode ePicMode)
{
    switch(ePicMode)
    {
        case eIFC_WAP_CINEMA:
            return eDDP_PICTURE_SETTINGS_VIDEO;

        case eIFC_WAP_BRIGHT:
            return eDDP_PICTURE_SETTINGS_BRIGHT;

        case eIFC_WAP_ENHANCED:
            return eDDP_PICTURE_SETTINGS_ENHANCED;

        case eIFC_WAP_REC709:
            return eDDP_PICTURE_SETTINGS_REC709;

        case eIFC_WAP_DICOMSIM:
            return eDDP_PICTURE_SETTINGS_DICOMSIM;

        case eIFC_WAP_BLENDING:
            return eDDP_PICTURE_SETTINGS_BLENDING;

        case eIFC_WAP_HDR:
            return eDDP_PICTURE_SETTINGS_HDR;

        case eIFC_WAP_3D:
            return eDDP_PICTURE_SETTINGS_3D;

        case eIFC_WAP_2DHIGHSPEED:
            return eDDP_PICTURE_SETTINGS_2DHIGHSPEED;

        case eIFC_WAP_USER:
            return eDDP_PICTURE_SETTINGS_USER;

        default:
            GEC_printf("(func:%s, line:%d) map fail (%d)\r\n", __FUNCTION__, __LINE__, ePicMode);
            return eDDP_PICTURE_SETTINGS_NUMBER; //A70LK_Casper_0001
