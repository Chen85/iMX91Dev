...
#include "halMCUCtrlAPI.h" //G100_Steven_0080
#include "halScaler.h"      //G100_Owen_0085
#include "halC789CtrlAPI.h" //G100_Owen_0085
...
#include "appEnvironment.h" //HICC2_Steven_0055
...
#define PCBA_LENGTH 30       //HICC2_Doulas_0126
...
static UINT8  m_ucPCBAbuf[PCBA_LENGTH] = {'\0'}; //HICC2_Doulas_0126
...
#endif
unsigned char cTester_PCBA(HPBU_TEST_COM_DATA *data); //HICC2_Doulas_0126
...
unsigned char cTester_SHUM(HPBU_TEST_COM_DATA *data); //HICC2_Julie_0007
unsigned char cTester_LSDT(HPBU_TEST_COM_DATA *data);
unsigned char cTester_TESP(HPBU_TEST_COM_DATA *data); //HICC2_Doulas_0126

unsigned char cTester_LSMV(HPBU_TEST_COM_DATA *data);  //HICC2_Steven_0055
...
static const  Rs232TesterCommLutTable sTable_TesterLut[] =
{
...
#endif
    {"PCBA", cTester_PCBA, UCLI_WRITABLE                   }, //HICC2_Doulas_0126
    ...
    {"SHUM", cTester_SHUM, UCLI_READABLE                   }, //HICC2_Julie_0007
    ...
    {"LSMV", cTester_LSMV, UCLI_READABLE                   },  //HICC2_Steven_0055
...
void DecodeCommand(char *src, HPBU_TEST_COM_DATA *dest)
{
    ...
    if(0 == strncmp(dest->chCommand, "PCBA", COMMAND_LENGTH)) //HICC2_Doulas_0126
    {
        for(i=0;i< dest->cDataSize;i++)
        {
...
// ==============================================================================
//G100_Clare_0069, mod, >>>
unsigned char cTester_VERG(HPBU_TEST_COM_DATA *data)
{
#ifdef _FIRMWARE_SYSTME
    return 0;
#else
    ...
    UINT8 aucFrontEndVer[16] = {0}; //HICC2_Doulas_0130
    ...
    sUtilHPBUTester_Callback.fpDataMgr_Data_AccessCb(edcFRONTEND_VERSION, edaREAD, aucFrontEndVer); //HICC2_Doulas_0130

    sprintf((char *)data->pcData, "%s-%s-%s-V%02d.%02d-%s-%s-%s\r\n",
        aucReleaseVer,
        aucFrontEndVer,  //HICC2_Doulas_0130
        aucFormatMCUVer,
        VER_MAJOR, VER_MINOR,
        aucKeypadVer,
        aucLANVer,
...
//G100_Clare_0069, mod, <<<
//G100_Clare_0069, add, >>>
unsigned char cTester_VEGG(HPBU_TEST_COM_DATA *data)
{
#ifdef _FIRMWARE_SYSTME
    return 0;
#else
    ...
    #ifdef PLATFORM_H60_2K //HICC2_Doulas_0018
    sprintf((char *)data->pcData, "%s-%s-%s-%s-%s-%s-%s\r\n",
        aucFPGA1Ver,
        aucFPGA3Ver,
        aucDDPVer,
        aucXFPGAVer,
...
//G100_Clare_0069, add, <<<
// ==============================================================================
// FUNCTION NAME: cTester_LSMV //HICC2_Steven_0055
// DESCRIPTION:
//
//
// Params:
...
unsigned char cTester_LDPS(HPBU_TEST_COM_DATA *data)
{
    ...
    halFormatter_PWM_Set(0, m_uiLD_PwmSetting[0]);
    halFormatter_PWM_Set(1, m_uiLD_PwmSetting[1]);
    halFormatter_PWM_Set(2, m_uiLD_PwmSetting[2]);
    halFormatter_PWM_Set(3, m_uiLD_PwmSetting[3]);
    halFormatter_PWM_Set(4, m_uiLD_PwmSetting[4]);
    halFormatter_PWM_Set(5, m_uiLD_PwmSetting[5]);
...
unsigned char cTester_LDPG(HPBU_TEST_COM_DATA *data)
{
    ...
    halFormatter_PWM_Update(); //A70LV_Larry_0084
...
unsigned char cTester_PRMG(HPBU_TEST_COM_DATA *data)
{
    ...
    // A70LV_Eric.C_0004 Start
    //UINT8 aucData[eFMT_MSG_LIQUID_SPEED_SZ]={0};
    UINT16 auiPumpSpeed[ePUMP_ID_NUMBERS]={0};
    //UINT8 ucCount = 0;
/*
    for(ucCount=0; ucCount<ePUMP_ID_NUMBERS; ucCount++)
    {
        halFanCtrl_Liquid_Speed_Get(aucData);

        auiPumpSpeed[aucData[0]] = ((UINT16)aucData[2] << 8) | (UINT16)aucData[1];
//        LOG_MSG(db_ALWAYS, "\r\n Pump_Speed         [%d] 0x%X 0x%X 0x%X",ucCount, aucData[0],aucData[1],aucData[2]);
    }

    sprintf((char *)data->pcData,"%d,%d\r\n",
        auiPumpSpeed[0],auiPumpSpeed[1]);
    // A70LV_Eric.C_0004 End
*/
	auiPumpSpeed[0] = palDataMgr_Access_Get_Pump();
...
unsigned char cTester_TECS(HPBU_TEST_COM_DATA *data)//A70Gen2_Julie_0042
{
	...
	{
		case MODEL_ID_0: //HICC2_Doulas_0034 Add  //MODULE_TYPE_ID0_PLATFORM
	        {
				if(uiVoltage[0] > 38000 && uiCurrent[0] < 1000)
				{
        ...
        case MODEL_ID_1: //HICC2_Doulas_0034 Add  //MODULE_TYPE_ID1_PLATFORM
	        {
				if(uiVoltage[0] > 38000 && uiCurrent[0] < 1000)
				{
					strcpy((char *)data->pcData, "TEC CHECK FAIL\r\n");
...
unsigned char cTester_LSEG(HPBU_TEST_COM_DATA *data)
{
    ...
    halMotor_ZoomLimit_Get(&sLimitZoom);
    halMotor_FocusLimit_Get(&sLimitFocus);
...
unsigned char cTester_LSDG(HPBU_TEST_COM_DATA *data)
{
    ...
    halMotor_ZoomLimit_Get(&sLimitZoom);
    halMotor_FocusLimit_Get(&sLimitFocus);
    halMotor_Lens_Backlash_Get(&sLensBacklash); //A70LV_Larry_0277
...
unsigned char cTester_LSPG(HPBU_TEST_COM_DATA *data)
{
    ...
    halMotor_LensCenterGet((BYTE*)&dwStep);
    sprintf((char *)data->pcData,"%d,%d\r\n", dwStep[0], dwStep[1]);
...
unsigned char cTester_OPTE(HPBU_TEST_COM_DATA *data)   //G100_Owen_0068
{
    halMotor_LensCenterEnable_Set(1);
    strcpy((char *)data->pcData, "Pass\r\n");
...
//    sUtilHPBUTester_Callback.fpDataMgr_Data_AccessCb(edcFILTER_WHEEL_INDEX, edaREAD, &uiFWIdx);
    halFormatter_PW_Index_Get(&uiPWIdx);
    halFormatter_FW_Index_Get(&uiFWIdx);
...
unsigned char cTester_LECA(HPBU_TEST_COM_DATA *data) //HICC2_Doulas_0056
{
    ...
    palMotorLensSet(eLENS_CMDS_LENS_ID_CAL);
    palMotorLensSet(eLENS_CMDS_ZOOM_CALIBRAION);
    palMotorLensSet(eLENS_CMDS_FOCUS_CALIBRAION);
...
// --------------------
// ==============================================================================
unsigned char cTester_DMDS(HPBU_TEST_COM_DATA *data)  //HICC2_Doulas_0098//HICC2_Julie_0007 DMD Humidity/Pressure sensor
{
#ifdef _FIRMWARE_SYSTME
    ...
    sUtilHPBUTester_Callback.fpDataMgr_Data_AccessCb(edcABSOLUTE_HUMIDITY2, edaREAD, (void*)aucAbsHumidity);

    sprintf((char *)data->pcData,"%s,%s,%s\r\n",aucPressSensor, aucHumidity, aucAbsHumidity);
...
unsigned char cTester_LSDT(HPBU_TEST_COM_DATA *data)
{
    ...
    UINT8 ucFujiDet = palMotor_LensFujiDet_Get();
