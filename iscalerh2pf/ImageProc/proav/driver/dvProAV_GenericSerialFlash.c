...
static int _FlashCsrWrite(SfcInfo *sfcinfo, uint08 csrAddr, uint32 wrData)
{
    int status = rcSUCCESS;
    status &= dvProAV_AccessWrite(sfcinfo->enumCsrAddr, (uint32)(csrAddr & 0x3F)); // Bit [5:0]
    status &= dvProAV_AccessWrite(sfcinfo->enumCsrWrData, wrData);
    status &= dvProAV_AccessWrite(sfcinfo->enumCsrCmd, 0x01); // 0x01: Write Mode, add this line to fixed two clock domain issue
    status &= dvProAV_AccessWrite(sfcinfo->enumCsrCmd, 0x03); // 0x03: Write Start
    status &= dvProAV_AccessWrite(sfcinfo->enumCsrCmd, 0x00); // 0x00: Finished
    return status;
}

static int _FlashCsrRead(SfcInfo *sfcinfo, uint08 csrAddr)
{
    int status = rcSUCCESS;
    uint32 rdData;

    status &= dvProAV_AccessWrite(sfcinfo->enumCsrAddr, (uint32)(csrAddr & 0x3F)); // Bit [5:0]
    status &= dvProAV_AccessWrite(sfcinfo->enumCsrCmd, 0x00); // 0x00: Read Mode, add this line to fixed two clock domain issue
    status &= dvProAV_AccessWrite(sfcinfo->enumCsrCmd, 0x02); // 0x02: Read Start
    status &= dvProAV_AccessWrite(sfcinfo->enumCsrCmd, 0x00); // 0x00: Finished
    status &= dvProAV_AccessRead(sfcinfo->enumCsrRdData, &rdData);
    return (int)rdData;
}

static int _SfiCmdDataWrite(SfcInfo *sfcinfo, uint8_t opcode,  uint32_t data, uint8_t dummy_count, uint8_t data_count)
{
    int status = rcSUCCESS;
    uint8_t data_type = 0;
    ...
    status &= _FlashCsrWrite(sfcinfo, REG_FLASH_CMD_SET, cmd);
    if (data_count > 0) status &= _FlashCsrWrite(sfcinfo, REG_FLASH_CMD_WD0, data);
    status &= _FlashCsrWrite(sfcinfo, REG_FLASH_CMD_CTL, 0x1);

    return status;
}

static int _SfiCmdDataRead(SfcInfo *sfcinfo, uint8_t opcode,  uint32_t *data_ptr, uint8_t dummy_count, uint8_t data_count)
{
    int status = rcSUCCESS;
    uint8_t data_type = 1;
    uint8_t address_count = 0;
    ...
    status &= _FlashCsrWrite(sfcinfo, REG_FLASH_CMD_SET, cmd);
    status &= _FlashCsrWrite(sfcinfo, REG_FLASH_CMD_CTL, 0x1);
    if (data_count > 0)  *data_ptr = _FlashCsrRead(sfcinfo, REG_FLASH_CMD_RD0);
...
static int _SfiCmdAddrWrite(SfcInfo *sfcinfo, uint8_t opcode, uint32_t address, uint8_t dummy_count, uint8_t address_count)
{
    int status = rcSUCCESS;
    ...
    status &= _FlashCsrWrite(sfcinfo, REG_FLASH_CMD_SET, cmd);
    status &= _FlashCsrWrite(sfcinfo, REG_FLASH_CMD_ADDR, address);
    status &= _FlashCsrWrite(sfcinfo, REG_FLASH_CMD_CTL, 0x1);
...
static int _SfiCmdAddrRead(SfcInfo *sfcinfo, uint8_t opcode, uint32_t *addr_ptr, uint8_t dummy_count, uint8_t address_count)
{
    ...
    uint8_t data_count = 0;
    uint32 cmd = (uint32_t)(dummy_count & 0x0F)<<16 | ((uint32_t)(data_count& 0x0F))<<12 | ((uint32_t)(data_type&0x01))<<11 | ((uint32_t)(address_count&0x07))<<8 | (uint32_t)(opcode&0xFF);
    _FlashCsrWrite(sfcinfo, REG_FLASH_CMD_SET, cmd);
    _FlashCsrWrite(sfcinfo, REG_FLASH_CMD_CTL, 0x1);
...
static int  _SfiMemoryReadStart(SfcInfo *sfcinfo)
{
    int status = rcSUCCESS;
    uint32_t mode;
    uint32_t data;
    ...
    {
        mode = 0x00000000;
        data = sfcinfo->b4BytesMode ? SFLASH_1X4B: SFLASH_1X; // Read opcode:0x03 // for 3 bytes addressing mode, polling opcode = 0x05
    }
    else
    {
        ...
        data = dummyCycle;
        data <<= 8;
        data |= sfcinfo->b4BytesMode ? SFLASH_4X4B: SFLASH_4X; // Read opcode:0x03 // for 3 bytes addressing mode, polling opcode = 0x05
    }
    status &= _FlashCsrWrite(sfcinfo, REG_OP_PROTOC_SET, mode);
    status &= _FlashCsrWrite(sfcinfo, REG_CONTROL, sfcinfo->b4BytesMode ? 0x00000101 : 0x00000001); // 3 or 4 byte address mode , CS0, Enable

    status &= _FlashCsrWrite(sfcinfo, REG_READ_INSTRU, data); // Read opcode:0x03 // for 3 bytes addressing mode, polling opcode = 0x05
    DBMSG("%s, Read Address Mode =%08X, \n", sfcinfo->acName, data);
    return status;
}

//Page Program Commands
static int _SfiMemoryWriteStart(SfcInfo *sfcinfo)
{
    int status = rcSUCCESS;
    uint32_t mode;
    uint32_t data;
    ...
    {
        mode = 0x00000000;
        data = sfcinfo->b4BytesMode ? (0x00000500 | SFLASH_PP4B) : (0x00000500 | SFLASH_PP); // Write opcode:0x02/0x12  // for 3 or 4 bytes addressing mode, polling opcode = 0x05
    }
    else
    {
        mode = 0x00000220;
        data = sfcinfo->b4BytesMode ? (0x00000500 | SFLASH_PP4X): (0x00000500 | SFLASH_PP4X); //Polling opcode = 0x05
    }
    status &= _FlashCsrWrite(sfcinfo, REG_OP_PROTOC_SET, mode);
    status &= _FlashCsrWrite(sfcinfo, REG_CONTROL, sfcinfo->b4BytesMode ? 0x00000101 : 0x00000001); // 3 or 4 byte address mode , CS0, Enable
    status &= _FlashCsrWrite(sfcinfo, REG_WRITE_INSTRU, data);
    DBMSG("%s, Write Address Mode =%08X, \n", sfcinfo->acName, data);
    return status;
}

inline static int _QuadModeEnable(SfcInfo *sfcinfo, bool enable)
{
    int status = rcSUCCESS;

    if (enable)
        status &= _SfiCmdDataWrite(sfcinfo, SFLASH_EQIO, 0, 0, 0);
    else
        status &= _SfiCmdDataWrite(sfcinfo, SFLASH_RSTQIO, 0, 0, 0);
    return status;
}

inline static int _MarconixQuadEnableCmd(SfcInfo *sfcinfo, bool enable)
{
    int status = rcSUCCESS;
    uint32 data;

    status &= _SfiCmdDataRead(sfcinfo, SFLASH_RDSR, &data, 0, 1);
    if (enable)
        data |= 0x40;
    else
        data &= ~0x40;

    status &= _SfiCmdDataWrite(sfcinfo, SFLASH_WRSR, data, 0, 1);
    return status;
}

inline static int _MicronQuadEnableCmd(SfcInfo *sfcinfo, bool enable)
{
    int status = rcSUCCESS;
    uint32 data;

    status &= _SfiCmdDataRead(sfcinfo, 0x65, &data, 0, 1); // 0x65 Read Enhanced Volatile Configuration Register
    if (enable)
        data |= 0x80;
    else
        data &= ~0x80;

    status &= _SfiCmdDataWrite(sfcinfo, 0x61, data, 0, 1); // 0x61 Read Enhanced Volatile Configuration Register
    return status;
}

inline static uint32_t _ReadDeviceID(SfcInfo *sfcinfo)
{
    uint32_t data;
    _SfiCmdDataRead(sfcinfo, SFLASH_RDID, &data, 0, 4);
    return data;
}

inline static uint32_t _ReadStatusRegister(SfcInfo *sfcinfo){
    uint32_t data;
    _SfiCmdDataRead(sfcinfo, SFLASH_RDSR, &data, 0, 2);
    return data;
}

inline static bool _IsWriteInProgress(SfcInfo *sfcinfo)
{
    uint32_t status = _ReadStatusRegister(sfcinfo);
    return ((status & MASK_SFLASH_WIP) == MASK_SFLASH_WIP); // 1: Write in progress (WIP) bit ( bit 0 of status register)
}

inline static bool _IsWriteEnableLatch(SfcInfo *sfcinfo)
{
    uint32_t status = _ReadStatusRegister(sfcinfo);
...
inline static bool _IsAllBlockProtected(SfcInfo *sfcinfo)
{
    uint32_t status = _ReadStatusRegister(sfcinfo);
...
inline static int _WriteEnableCmd(SfcInfo *sfcinfo)
{
    return _SfiCmdDataWrite(sfcinfo, SFLASH_WREN, 0, 0, 0);
}

inline static int _WriteDisableCmd(SfcInfo *sfcinfo)
{
    return _SfiCmdDataWrite(sfcinfo, SFLASH_WRDI, 0, 0, 0);
}

//Erase Commands
inline static int _EraseBlock(SfcInfo *sfcinfo, uint32_t address)
{
     return _SfiCmdAddrWrite(sfcinfo, SFLASH_BE, address, 0, 4); // SFLASH_BE: 0xD8 is Micron block erase(64K) command as the Marconix block erase(64K) command
}

inline static int _EraseChip(SfcInfo *sfcinfo){
     return _SfiCmdDataWrite(sfcinfo, SFLASH_CE, 0, 0, 0);
}
...
{
    return _SfiCmdDataWrite(sfcinfo, SFLASH_EN4B, 0, 0, 0);
}
...
{
     return _SfiMemoryReadStart(sfcinfo);
}
...
int dvProAV_GenericSfiWriteMemoryCmd(SfcInfo *sfcinfo){
     return _SfiMemoryWriteStart(sfcinfo);
}

//Block Protection Commands
inline static int _AllBlockUnprotectCmd(SfcInfo *sfcinfo)
{
    int status = rcSUCCESS;
    uint32 data;
    ...
    status &= _SfiCmdDataRead(sfcinfo, SFLASH_RDSR, &data, 0, 1);
    data &= ~0x3C;
    status &= _SfiCmdDataWrite(sfcinfo, SFLASH_WRSR, 0x00, 0, 1); //sector unprotect all sector(BP3:BP2:BP1:BP0=0:0:0:0)
    return status;
}

inline static int _AllBlockProtectCmd(SfcInfo *sfcinfo)
{
    int status = rcSUCCESS;
    uint32 data;
    ...
    status &= _SfiCmdDataRead(sfcinfo, SFLASH_RDSR, &data, 0, 1);
    data |= 0x3C;
    status &= _SfiCmdDataWrite(sfcinfo, SFLASH_WRSR, data, 0, 1); //sector protect all sector(BP3:BP2:BP1:BP0=1:1:1:1)
    return status;
}
...
int dvProAV_GenericSfiAllSectorUnprotect(SfcInfo *sfcinfo)
{
   ...
   //d. Performs read status register command to check whether BP bit and TB bit	has succeeded clear.
    ...
    if (_IsWriteEnableLatch(sfcinfo) == false) {
        return rcERROR;
    }

    _AllBlockUnprotectCmd(sfcinfo); //sector unprotect all sector(BP3:BP2:BP1:BP0=0:0:0:0)
    ResetTime(ucTimerId);
    while (_IsWriteInProgress(sfcinfo)){
...
int dvProAV_GenericSfiEraseSector(SfcInfo *sfcinfo, uint32_t address, uint32 length)
{
    ...
    dvProAV_GenericSfiAllSectorUnprotect(sfcinfo);

    dvProAV_GenericSfiEnter4byteAddressingMode(sfcinfo);
    ...
    {
        _EraseBlock(sfcinfo, adr - (adr%FLASH_SECTOR_SIZE));
    }

    ResetTime(ucTimerId);

    //c. Polls write in progress (WIP) bit ( bit 0 of status register) until it return a 0 (ready).
    while (_IsWriteInProgress(sfcinfo)){   //wait sector erase to end before proceed to next
    ...
    dvProAV_GenericSfiAllSectorProtection(sfcinfo);
    return status;
}
...
{
    int status = rcSUCCESS;
    _FlashCsrWrite(sfcinfo, REG_READ_CAPTURE, ulDly);
    return status;
}
...
{
    int status = rcSUCCESS;
    _FlashCsrWrite(sfcinfo, REG_SPI_CLOCK_BRT, ulSpiClockRate); // set SPI clock
    return status;
}
...
int dvProAV_GenericSfiInit(SfcInfo *sfcinfo)
{
    ...
    DBMSG("dvProAV_GenericSfiInit for %s\n", sfcinfo->acName);

    dvProAV_AccessWrite(sfcinfo->enumIpReset, 1);
    dvProAV_AccessWrite(sfcinfo->enumIpReset, 0);

    dvProAV_AccessWrite(sfcinfo->enumReset, 1);
    dvProAV_AccessWrite(sfcinfo->enumReset, 0);


    DelayMSec(500);

    _FlashCsrWrite(sfcinfo, REG_CS_DELAY_SET, sfcinfo->u32CsDelay); // set CS assert & de-assert both delay
    _FlashCsrWrite(sfcinfo, REG_SPI_CLOCK_BRT, sfcinfo->u32SpiClockRate); // set SPI clock
    data = _ReadDeviceID(sfcinfo);
    ...
    if (data == 0xc23b25c2 || data == 0xc23b95c2) // Mxic 0xc23b25c2:SOP , 0xc23b95c2:BGA
    {
        ...
        if (sfcinfo->ioMode == 1)
        {
            _MarconixQuadEnableCmd(sfcinfo, true);
        }
    }
    else if (data == 0x1021bb20)
    {
        ...
        if (sfcinfo->ioMode == 1)
        {
            _MicronQuadEnableCmd(sfcinfo, true);
        }
    }
    else // Unknow Flash
    {
    ...
    _IsAllBlockProtected(sfcinfo);

#if 0
    _SfiCmdDataRead(SFLASH_CMD_TOP, &data, 0, 1);
    ...
    _SfiCmdDataRead(SFLASH_CMD_4BYTE, &data, 0, 1);
