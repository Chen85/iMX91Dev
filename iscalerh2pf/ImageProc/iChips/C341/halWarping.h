...
#define WARPING_TYPE_ADVANCED               (0xB0)    //Advanced //WARPING_TYPE_OSD
#define WARPING_TYPE_BASIC                  (0xB1)    //Basic //WARPING_TYPE_OSD_MYSTIQUE
...
#ifdef SCALER_C341
#define WARPING_COLOR_UNIFORMITY_SIZE       0x00001EB4 //Ch0 and Ch1
#else
...
//===== 4 Corner Based =====//
#define FOUR_CORNER_Based_H         3840
#define FOUR_CORNER_Based_V         2400
...
#define WARP_OSD_PALETTE_ELEMENT (3)
#define WARP_OSD_BLINK_COLOR_MAX (2)


#define DEFAULT_WARP_TABLE_CRC_3840x2400 64360
...
}eHAL_WARPING_MOVE_PITCH;

typedef enum
{
    eWARP_EVENT_WARPOFF         = 0x00,
    eWARP_EVENT_KEYSTONE_H      = 0x01,
    eWARP_EVENT_KEYSTONE_V      = 0x02,
    eWARP_EVENT_KEYSTONE_HV     = 0x03,
    eWARP_EVENT_PINBARREL_H     = 0x04,
    eWARP_EVENT_PINBARREL_V     = 0x05,
    eWARP_EVENT_PINBARREL_HV    = 0x06,
    eWARP_EVENT_4C_TL_X         = 0x07,
    eWARP_EVENT_4C_TL_Y         = 0x08,
    eWARP_EVENT_4C_TR_X         = 0x09,
    eWARP_EVENT_4C_TR_Y         = 0x0A,
    eWARP_EVENT_4C_BL_X         = 0x0B,
    eWARP_EVENT_4C_BL_Y         = 0x0C,
    eWARP_EVENT_4C_BR_X         = 0x0D,
    eWARP_EVENT_4C_BR_Y         = 0x0E,
    eWARP_EVENT_4C_ALL          = 0x0F,
    eWARP_EVENT_CURVE_L         = 0x10,
    eWARP_EVENT_CURVE_R         = 0x11,
    eWARP_EVENT_CURVE_T         = 0x12,
    eWARP_EVENT_CURVE_B         = 0x13,
    eWARP_EVENT_ROTATION        = 0x14,

    eWARP_EVENT_DEFAULT_FILTER_H = 0x15,
    eWARP_EVENT_DEFAULT_FILTER_V = 0x16,

    eWARP_EVENT_WARP_H_FILTER_USER_SEL = 0x17,
    eWARP_EVENT_WARP_V_FILTER_USER_SEL = 0x18,
    eWARP_EVENT_WARP_H_FILTER_AP_SEL = 0x19,
    eWARP_EVENT_WARP_V_FILTER_AP_SEL = 0x20,
    eWARP_EVENT_USER_WARP_FILTER_AUTO_ENABLE = 0x21,
    eWARP_EVENT_AP_WARP_FILTER_AUTO_ENABLE = 0x22,
...
typedef struct
{
    ...
    sBLACKLEVEL_PARA asBlackLevelPara[MAX_BLACKLEVEL_AREA_NUMBER];
...
eHAL_WARPING_EXEC_CODE halWarping_SetWarp_AutoFilterParameter(UINT8 ucAutoFilter,UINT8 *ucHorzFilter,UINT8 *ucVertFilter);
eHAL_WARPING_EXEC_CODE halWarping_SetWarp_ManualFilterParameter(UINT8 ucHorzFilter,UINT8 ucVertFilter);


eRESULT halWarp_GeometrySet(void);
void halWarp_GeometryParameterSet(eWARP_EVENT eType, void *pcData);
void halWarp_GeometryParameterGet(eWARP_EVENT eType, void *pcData);
...
eHAL_WARPING_EXEC_CODE halWarp_BlendingSet(void);
UINT16 halWarp_EdgeBlendingSet(sEDGE_BLENDING_PARA sEdgeBlending);
eHAL_WARPING_EXEC_CODE halWarp_BlendingGammaSet(void);
eHAL_WARPING_EXEC_CODE halWarp_BlendingGammaCoefSet(float fCoef);

void halWarping_TwistLinkFlag_Set(BOOL bTwistLink);
BOOL halWarping_TwistLinkFlag_Get(void);
...
eHAL_WARPING_EXEC_CODE halAdvBlackLevelSelectControlPoint(UINT8 ucValue);
eHAL_WARPING_EXEC_CODE halAdvBlackLevelMoveControlPoint(UINT8 ucValue);
eHAL_WARPING_EXEC_CODE halAdvBlackLevelAddPoint(void);
eHAL_WARPING_EXEC_CODE halAdvBlackLevelDeletePoint(void);
eHAL_WARPING_EXEC_CODE halAdvBlackLevelSelectArea(UINT8 ucValue);
eHAL_WARPING_EXEC_CODE halAdvBlackLevelSetEnable(UINT8 ucValue);
eHAL_WARPING_EXEC_CODE halAdvBlackLevelSetRed(UINT8 ucValue);
eHAL_WARPING_EXEC_CODE halAdvBlackLevelSetGreen(UINT8 ucValue);
eHAL_WARPING_EXEC_CODE halAdvBlackLevelSetBlue(UINT8 ucValue);
eHAL_WARPING_EXEC_CODE halAdvBlackLevelReset(UINT8 ucResetLevel);
eHAL_WARPING_EXEC_CODE halAdvBlackLevelShowAddPointPattern(void);
eHAL_WARPING_EXEC_CODE halAdvBlackLevelShowRemovePointPattern(void);
BOOL halAdvBlackLevelIsPointDeletable(void);
BOOL halAdvBlackLevelIsPointAddable(void);
...
eHAL_WARPING_EXEC_CODE halWarping_AP_SaveInfoForCurrentFile(void);
eHAL_WARPING_EXEC_CODE halWarping_AP_LoadCurrentFileInfo(sCurrentFileInfo *psCurrentInfo);
...
eHAL_WARPING_EXEC_CODE halAdvHide_OSD_Pattern_Set(BOOL bHide);
eHAL_WARPING_EXEC_CODE halAdvBlack_Level_Boundary_Pattern_Hide(BOOL bHide);
...
eHAL_WARPING_EXEC_CODE halAdvWarpShowOsdPattern_Unblocked_TwistLink(UINT8 ucValue);
...
eHAL_WARPING_EXEC_CODE halAdvBlack_Level_Boundary_Flag_Set(UINT8 ucBoundary);
eHAL_WARPING_EXEC_CODE halWarp_AP_Set_Interpolation_Lut_H_Index(UINT8 ucHorzPolationIdx);
eHAL_WARPING_EXEC_CODE halWarp_AP_Set_Interpolation_Lut_V_Index(UINT8 ucVertPolationIdx);
eHAL_WARPING_EXEC_CODE halWarping_AdaptiveScaleFilterLutInit(void);
eHAL_WARPING_EXEC_CODE halWarping_AdaptiveScaleFilterLutEnable(UINT8 ucEnable);
eHAL_WARPING_EXEC_CODE halWarping_CurrentWarpTypeSet(UINT8 ucType);
...
void halWarping_FrameMemoryProcessCheck(UINT8 IsDefaultWarpTable);
void halWarping_IsDefaultWarpTable_Set(UINT8 IsDafault);
