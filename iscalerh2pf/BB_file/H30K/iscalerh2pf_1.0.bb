...
do_compile() {
        ...
        ${CC} -D${PLATFORM_DEF} -D${CUSTOM_DEF} -D${SCALER_TYPE} -D${OUTPUT_PANEL_CFG} -D${DMD_RESOLUTION} -DLow_Latency_All${CFLAGS} -DOPFU_TAG_ENABLE\
        ...
        elapsed_sec=$(python3 -c 'import sys; s=int(sys.argv[1]); e=int(sys.argv[2]); print("{:.3f}".format((e-s)/1000.0))' \
                             "${start_time}" "${end_time}")

        bbplain "-----   compile time : ${elapsed_sec} seconds"
        bbplain "-----------------------------------------------------"
        bbplain "-----------------------------------------------------"
}
